...
│   ├── enemy_attack_component.gd / .tscn  Enemy committed melee: proximity windup → hold on lunge frame → rect hitbox + KnockbackComponent lunge + per-enemy attack SFX; editor hitbox_size/offset; Chill speed_multiplier + stun cancel
...
- `project/components/health_bar_component.gd` — world-space 18×2 px `_draw` bar; fill is `% of max_health` (same pixel width for every entity); hidden until `damage_taken`, then visible for 1.5 s (timer refreshes on each hit); `@export` offset/colors; instanced on player, goblin, brute, and cactus
...
- `project/components/status_component.gd` — enemy statuses: **Poison** (every 1 s deal `stacks` HP; stacks persist until death; ticks call `take_damage(..., POISON)` for green labels) and **Shock** (at 10 stacks deal 50 HP + stun 2 s via `NavigationComponent.set_chasing(false)` + `MovementComponent.stop()` + optional `EnemyAttackComponent.cancel()`, then clear Shock; no Shock while stunned) and **Burn** (on `DestroyComponent.destroyed`, explode for `5 * stacks` in `50 * (1 + 5% * stacks)` px; enemies only; BURN labels) and **Chill** (`5% * stacks` move + attack-speed slow, max 90%; slows `EnemyAttackComponent.speed_multiplier` when present, else stretches `DamageComponent.contact_damage_interval`) and **Disease** (permanent flag + sickly-green `rest_modulate` tint; on death, nearby enemies within 100 px gain `floor(poison/2)` Poison stacks; no Disease chaining); shared `_nearby_enemies` shape query for Burn + Disease; `add_stacks(id, amount, source)` tracks per-status source for drops/explosions; instanced on goblin/brute with health/nav/movement/destroy wired (goblin also wires `attack_component`; brute still wires `damage_component`)
...
- `project/components/enemy_attack_component.gd` — enemy `Area2D` committed melee; editor `hitbox_size` / `hitbox_offset` build a per-instance `RectangleShape2D`; proximity → windup anim → hold on `lunge_frame` → attack SFX + live hitbox + `KnockbackComponent.push_distance` lunge → recover/cooldown; `set_active` / `cancel` / `speed_multiplier`; editor `@tool` gizmo outline
