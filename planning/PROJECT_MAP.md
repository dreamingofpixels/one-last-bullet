...
│   ├── damage_component.gd / .tscn    Damage + instigator + optional contact tick interval + damage_kind
    ...
    │   ├── status_component.gd / .tscn   Enemy Poison (1 dmg/stack/s, persistent) + Shock (stun + 50 burst at 10 stacks) + Burn (death explosion) + Chill (move/attack slow) + Disease (permanent; on death spreads floor(poison/2) within 100 px)
...
│   ├── collision_separation.gd      Static shape-query helpers: is_clear / resolve_along / separate (dash unstick, player watchdog, enemy spawn depenetration)
    ...
    │   │   ├── contagion/contagion_orb.tscn / .gd      Impact + Poison from GameData; 20% Disease on hit; Disease death spreads half poison
...
- `project/components/status_component.gd` — enemy statuses: **Poison** / **Shock** / **Burn** / **Chill** / **Disease** (see DECISIONS); Chill slows `EnemyAttackComponent.speed_multiplier` when present, else stretches `DamageComponent.contact_damage_interval`; instanced on goblin/ogre with health/nav/movement/destroy + `attack_component` wired
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `uses_mouse()` (P1 only); `get_move_vector()`, `get_aim_vector(origin)`, `is_attack_just_pressed()`, `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()`
...
- `project/objects/summoning_circle/summoning_circle.tscn` + `.gd` — floor circle with `%DepositArea` (radius 22), `%InfoProximityArea` (radius 64, player-only, OrbInfo and BuyBlankOrb visibility), `%ManaPoolLabel`, `%ArcaneParticles`, `%OrbInfo` (`%OrbNameLabel` + `%EffectLabel` + stats; `%FireHint` / `%WaterHint` / `%AirHint` / `%EarthHint` on each element row — shown only at exactly two socketed glyphs via `OrbRecipes.hints_for`; at three playable glyphs OrbInfo name/effect/stats preview the specialist), `%GlyphSlots` / `%GlyphSocket1-3` / `%GlyphIcon1-3` (`socket_interact_radius` 12 for Cross place/remove; GlyphSlots use `z_as_relative = false` like the floor sprite so they draw behind the player), `%BuyBlankOrb` (idle / waiting, only while a player is in InfoProximityArea); owns `mana_pool` (no glyph inventory); **`MAX_ORBS` 5**; live ritual: Square/`activate` arms capture, Triangle/`upgrade` buys or Transform/bake (Transform launches via `commit_transformed_orb`); signals `ritual_started` / `ritual_ended` / `new_blank_orb_requested` / `transform_requested`; group `summoning_circle`
...
- `project/globals/orb_recipes.gd` — **`OrbRecipes`**: unordered element-multiset lookups over `orbs` / `attunements`; `hints_for` / `result_for` / `is_playable` (`resolved_scene_path`: authored path if the file exists, else `res://entities/orbs/{id}/{id}_orb.tscn`) / `stats_from_row` / `effect_text` / provisional `is_discovered`
...
| `pickup` | right click, gamepad **X / Cross** (device 0, button 0) | gamepad **X** device 1 |
