...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (no freeze while charging; catch on release — wedge tap / spin ring; launch on contact frame); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / fixed-pitch crack at contact / pitched `orb_redirect` at fly-off); **pickup** glyph grab/throw + ritual socket
