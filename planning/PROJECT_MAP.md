...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (charge → swing polygon or 360° at `attack_spin_radius`; move/dash locked while charging; snapshot at release, freeze on contact frame + `attack_bat_hold_seconds` before deflect); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / fixed-pitch crack at contact / pitched `orb_redirect` at fly-off); **pickup** glyph grab/throw + ritual socket
...
│   └── objects/
        ...
        ├── summoning_circle/
        │   └── summoning_circle.tscn / .gd / .png   Live ritual OrbInfo + GlyphSlots; mana; opening orb origin
        ├── cactai/
...
- `project/areas/level/level.gd` — director: bakes navigation, starts `EnemySpawner`, awaits parallel `begin_level()`; applies `RunStartConfig` to the circle then launches configured orbs (default **Ghost + Contagion + Conduit**) from `%SummoningCircle` into `%WorldYSort` with a brief mutual collision exception (~0.2 s) so co-spawned orbs fan out; on kills rolls **glyph** spawn via orb `glyph_drop` or `glyph_drop_chance` export; weighted glyph rarity **70 / 25 / 5**; `SummoningCircle.ritual_started` → live circle UI (no pause); `new_blank_orb_requested` / `transform_requested` from circle (Transform instantiates the specialist then `commit_transformed_orb` launches it); `pause` → `%RitualMenu.open_inspect`; co-op P2 hot-join; win/lose when all players dead; orb tether/time-slow hooks; redirect mode comes from each player’s `character` (not overwritten by the level)
...
- `project/components/attack_component.gd` — player `Area2D` arc; authored `%CollisionPolygon2D`; `overlaps_world_circle()` / `contains_world_point()` for Attack bat geometry (orb body disk vs wedge); persistent orange `_draw` wedge while `attack_bat_enabled`; swaps to **`attack_spin_radius`** circle when `is_attack_charge_ready()` or during 360° spin (`begin_spin_overlay`); **`flash_bat_hit()`** whites the wedge/circle for ~0.15 s on a connecting bat; rotation follows `OrbTetherComponent.get_bat_aim()` (right stick / mouse; latches last aim, not move/facing); **`get_visual_forward(aim)`** is the polygon-centroid direction the dwarf body faces; AnimationPlayer swing; knocks enemies via `KnockbackComponent` when `melee_enabled` (physics hitbox unused for bat); orb deflect behind `deflect_orb_enabled` (default false); `play_swing_visual(aim)` plays arc sprite without hitbox; `consume_cooldown()` for Attack bat / proximity redirect; hides `%AttackSpriteHint` while bat overlay is on or melee is off; optional `swing_sound` (unassigned)
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`attack_spin_radius` (32 px)** for charged 360° bat (body-disk overlap: spin radius + orb radius); `try_attack_bat` **snapshots** overlaps at release; freeze / flash / squash / pose pause / fixed-pitch crack on **contact frame** (`attack_bat_contact_frame` **3** / `attack_bat_spin_contact_frame` **1**); **`attack_bat_hold_seconds` (0.15)** orb-only hitstop (multi-orb `_bat_holds`; live tap aim / spin bounce unless stick; delayed `deflect` + speed-pitched `orb_redirect`); **`orb_redirect_mode`** (`ATTACK_BAT` / `DASH_VAULT`, mutual exclusive; set by player `character`); **`capture_enabled`** (false on player) gates tap capture + remote channel; **attack** charge hold (**move/dash locked**) then `try_attack_bat(front_cone)` — short release = swing **polygon** + `attacking`, hold ≥ 0.5 s = 360° + `attack_around`; `get_bat_aim()` shared with overlay (stick/mouse, latches last aim); plays `attack_bat_sound` (axe whoosh) once per bat, fixed-pitch crack at contact (`attack_bat_impact_pitch`), and speed-pitched send at fly-off; **pickup** press order: release owned tether → throw if carrying → **ritual socket/unsocket** if circle has a captured orb → **glyph** pickup → orb capture (if capture on); circle **activate** is on **`activate`** (Triangle / Space) including Transform/bake; buy on **`upgrade`** (Square / E); …
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `InputScheme` last-input tracking (`KEYBOARD_MOUSE` / `GAMEPAD`; P1 defaults kb/mouse, P2+ gamepad); `prefers_keyboard_mouse()` for prompts; `uses_mouse()` (P1 only, GUI gates); `get_move_vector()`, `get_aim_vector(origin)`, `get_explicit_aim_vector(origin)` / `get_stick_aim_vector()` / `is_explicitly_aiming()` (bat wedge/chevron + launch; stick-only for spin bat override); `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()` / `is_attack_just_pressed()` / `is_attack_pressed()` / `is_attack_just_released()` / activate / upgrade / ritual_cancel
...
- Player facing: 8-way logical facing; visuals use authored `<action>_sw` / `<action>_nw` (`idle` / `moving` / `attacking` / `attack_around`) with `flip_h` for SE/NE; missing NW falls back to SW; `DirectionalSpriteComponent` owns octant snap, N/S flip from aim X (12/6 o'clock), E/W N/S bias, east flip, and charge pause-hold; **wizard** faces movement while walking; **dwarf** faces the wedge visual center (`Player.sync_body_facing` / `AttackComponent.get_visual_forward`); redirect/throw call `Player.play_attack_visual()` (dwarf spin uses `play_attack_around_visual()`).
