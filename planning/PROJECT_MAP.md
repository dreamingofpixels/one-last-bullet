...
│   ├── attack_component.gd / .tscn   Player arc swing; `%CollisionPolygon2D` geometry hit + persistent overlay for Attack bat; hitbox/knockback behind melee_enabled; `play_swing_visual` for arc art; orb deflect gated by flag; Attack bat consumes cooldown
...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (charge → swing polygon or 360° at `attack_spin_radius`; move/dash locked while charging); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / `orb_redirect`); **pickup** glyph grab/throw + ritual socket
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `InputScheme` last-input tracking (`KEYBOARD_MOUSE` / `GAMEPAD`; P1 defaults kb/mouse, P2+ gamepad); `prefers_keyboard_mouse()` for prompts; `uses_mouse()` (P1 only, GUI gates); `get_move_vector()`, `get_aim_vector(origin)`, `get_explicit_aim_vector(origin)` / `is_explicitly_aiming()` (bat wedge/chevron + launch), `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()` / `is_attack_just_pressed()` / `is_attack_pressed()` / `is_attack_just_released()` / activate / upgrade / ritual_cancel
