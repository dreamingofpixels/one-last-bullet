...
- **Decision**: Gameplay sprites that should depth-sort (player, enemies, props, earth spikes) are direct children of `%WorldYSort` (`y_sort_enabled`). Shockwave line controllers stay on the level root; spikes are parented into `%WorldYSort` (never the scene root — that draws above the whole group). Earth spikes bias their y-sort point 24 px north of the visual base so center-origin entities at the same ground line draw in front. Navmesh `navigation_source` stays on tiles/walls and on static prop scenes (not on WorldYSort) so the player/enemies are not baked as obstacles.
- **Why**: Godot y-sort only compares siblings; folder wrappers at (0,0) made spikes unable to sort against entities. A shared sort root fixes spike vs player/prop overlap.
- **Alternatives**: Per-node `z_index = int(y)` — works without a shared parent but needs every entity updated; y-sort only on the line controller — sorts spikes among themselves only; keep Objects/Players/Enemies wrappers under a sort root — wrappers share Y=0 so sorting does nothing.
- **Status**: decided (in-codebase)
...
- **Decision**: Destructible cliff segments are a dedicated `Cliff` breakable (`cliff.tscn` / `cliff.gd`). Authors pick `kind` among seven sheets — `corner_SW` / `corner_SE` / `wall_1` / `wall_2` / `corner_NW` / `corner_NE` / `vertical_wall` — and may preview `frame_index` in the inspector; an `@tool` apply sets the vertical-strip `Sprite2D` (`hframes` 1 / `vframes` 4 for corners and horizontals, **2** for `vertical_wall` at 32×32 cells) plus body/hitbox rects. Does **not** call `LevelObject._ready()` / random `LevelObjectVariant`. Default HP **80**. Optional collision size/offset overrides. **At runtime**, adjacency kinds overwrite `frame_index` from 32 px neighbors (group `cliffs`; left / right / above / below, 4 px slop). `vertical_wall` skips adjacency framing and instead picks `randi() % 2` once on ready (still counts as a neighbor for others). `Cliff.refresh_all_frames` runs deferred on ready and when any cliff is destroyed (dying cliff leaves `cliffs` immediately and is skipped if queued for deletion). Frame rules: SW/NW corners map both-connected → 0, either relevant side → 1, neither → 3; SE/NE distinguish the two single-neighbor cases as 1 vs 2; horizontals use left+right (0–3).
- **Why**: Kind is placement authorship; frame must match living neighbors so destroying a segment reveals the correct cut faces without hand-wiring each instance. Vertical fill pieces only need cosmetic variety, not expose-side logic. Editor frame remains for layout preview before play.
- **Alternatives**: Separate `vertical_wall_W` / `vertical_wall_E` with above/below adjacency — superseded by one randomized `vertical_wall` sheet; TileMap terrain peering — cannot carry per-piece HP / destroy VFX; random `LevelObjectVariant` — wrong for authored cliff shapes.
- **Status**: decided (in-codebase)
