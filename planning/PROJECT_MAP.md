...
│   ├── dash_component.gd / .tscn     Fixed-distance dash with i-frames, ghost alpha, afterimages, dash SFX, + 4s cooldown ring (reset on Attack redirect)
    ...
    │   ├── chaos_orb/
    │   │   ├── chaos_orb.tscn / chaos_orb.gd   Shared ChaosOrb base (FLYING/TETHERED/POSSESSED); not launched at level start
    │   │   ├── shadow_orb.tscn / .gd          Possess host on hit; 3 DPS; dark flash; emerge on death
    │   │   ├── poison_orb.tscn / .gd          Impact + 1 Poison stack on enemy hit
    │   │   ├── electric_orb.tscn / .gd        Impact + current beam (Line2D / capsule Area2D) within 300 px of player
    │   │   ├── chaos_orb_legacy.tscn / chaos_orb_legacy.png  (kept as alternate; script shared)
    │   │   ├── orb.png / orb_in_focus.png / aim_arrow.gd
...
- `project/entities/player/states/idle.gd` — stops movement; transitions to walk, dash; attack throws a carried crystal first, else tries `try_redirect_attack()` (stays idle on success); out-of-range Attack does nothing while melee parked; skips Attack when pointer is over blocking GUI; attack blocked while tethering; walk/dash blocked while tethering
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()`; transitions to idle, dash; attack throws a carried crystal first, else tries `try_redirect_attack()` (stays walk on success); out-of-range Attack does nothing while melee parked; skips Attack when pointer is over blocking GUI; attack blocked while tethering; forces idle while tethering
...
- `project/entities/chaos_orb/aim_arrow.gd` — drawn aim arrow (kept dormant; redirect preview uses `%RedirectParticles`)
