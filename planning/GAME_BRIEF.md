...
- **Attack**: press attack to swing a **90° arc** (radius 16 px) toward the mouse / right stick. The orb **bounces off** the arc; enemies in the arc get **knocked back**. Movement stays unlocked during the swing.
...
- Mid-combat steer: attack arc deflects the orb off a radial normal; brief post-deflect grace so the player is not instantly killed. Attack has a short cooldown (~0.35s).

## Input
- **Move**: WASD (keyboard) or left stick / D-pad (gamepad)
- **Attack**: left click or Space (keyboard); A button (gamepad)
