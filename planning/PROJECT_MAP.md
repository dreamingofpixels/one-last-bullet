...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (no freeze while charging; catch on release — wedge tap / spin ring∪wedge; launch on contact frame; charge **0.4 s**); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / fixed-pitch crack at contact / pitched `orb_redirect` at fly-off); **pickup** glyph grab/throw + ritual socket
│   ├── dash_component.gd / .tscn     Fixed-distance dash with i-frames, ghost alpha, afterimages, dash SFX, cooldown ring; vault catch via `end_and_clear_cooldown()`; end-of-dash CollisionSeparation so phase-through never restores inside world geometry
...
- Player character: inspector `character` on `player.tscn` picks wizard (dash vault + existing frames) or dwarf (Attack bat + `dwarf_frames.tres`).
