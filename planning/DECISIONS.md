...
- **Decision**: `HealthComponent.max_health` defaults to `1.0`, while concrete HP values are authored per scene/entity. Current examples: player `30`, grunt `20`, brute `50`, cactus `5`, `big_rock` `60`, `animal_skull` `20`. Orb-hit damage always comes from the orb `DamageComponent.damage` (currently `10`) via `_try_apply_orb_damage` on flying `body_entered` and tether contact paths. On any `take_damage`, the entity sprite modulates to red (`damage_flash_color`) then tweens back (non-fatal) or stays red into the destruction FX (fatal). Sprite comes from an optional `HealthComponent.sprite` export, else `DestroyComponent.sprite`. Non-fatal hits can start gameplay i-frames (see below).
...
- **Decision**: Hit detection for **non-orb** attackers uses `HitboxComponent` (`Area2D`) on both attackers and victims. Each physics frame the victim hitbox polls `get_overlapping_areas()`, resolves the attacker's `COMPONENTS[DamageComponent]`, and applies damage when the overlap is fresh (or when a contact-damage interval elapses). A short `hit_dedup_frames` grace (default 2) keeps an attacker "seen" after leaving so bounce flicker does not count as a new hit. Entries prune themselves once past that grace. **Orb outgoing damage** is applied on `RigidBody2D.body_entered` while flying (and on the tether shape probe / hitbox-area path while tethered) via `_try_apply_orb_damage`, so CCD bounce and HP share the same contact; victim hitboxes **skip** attackers in the `orb` group so a full overlap cannot double-hit. Breakables and entities with `HealthComponent` share that body-contact path so the bounce impulse resolves before `queue_free()`.
- **Why**: Decouples enemy contact-tick registration from physics enter/exit chatter; sustained enemy contact can tick repeatedly; self-cleaning state avoids unbounded cooldown dictionaries. Body-contact orb damage closes the gap where `_integrate_forces` bounce + CCD depenetration left Area2D with no overlapping sample on the same frame.
- **Alternatives**: `area_entered` only — one damage forever while glued, and bounce flicker double-hits; wall-clock per-attacker cooldown — magic number that can swallow legitimate late-run hits; signal bus — extra indirection; keep orb damage on victim hitbox poll — misses grazes after script bounce; poke victim `_attacker_state` from the orb after body damage — couples the two systems.
- **Status**: decided (in-codebase)
...
- **Decision**: `ChaosOrb` is a `RigidBody2D` with a circular body shape (world + player + enemy mask), locked rotation, gravity 0, friction 0, **bounce 0** material, continuous CCD, `can_sleep` off, and constant-speed flight. All reflection is owned by `_integrate_forces`: inbound contact normals are summed, then `aim_direction` reflects **once** off that combined normal (only when moving into the surface), then `linear_velocity = aim_direction * speed`. `_physics_process` does **not** re-derive direction from solver velocity. Flying damage and bounce SFX use `body_entered` (`_try_apply_orb_damage` for any `HealthComponent` root). The orb's `HitboxComponent` Area2D remains for tethered overlap / forced break, not as the flying damage source.
- **Why**: Solver bounce + script bounce fought each other and caused sticky re-entry (double damage / double SFX). Real contact normals beat center-to-center approximations on capsules and rectangles. Single bounce authority keeps the projectile readable. Body-contact damage keeps HP tied to the same contact that bounces.
- **Alternatives**: PhysicsMaterial bounce 1 + velocity read-back — previous; double-reflect artifacts; radial bounce only in `body_entered` for CharacterBody2D — wrong normals and still fought the solver; world-only mask + area-driven entity bounce — reverses entity-body bounce feel; wall-clock hit cooldown — papers over the symptom; flying damage only via victim hitbox poll — grazes bounce with no HP after CCD depenetration.
- **Status**: decided (in-codebase)
...
- **Decision**: Breakables (cactus) live on the `world` physics layer like rocks/walls. The orb damages them via the same flying `RigidBody2D.body_entered` → `_try_apply_orb_damage` path used for enemies/player (not the Area2D hitbox poll), so the bounce impulse is solved before `queue_free()`.
- **Why**: Props should reshape orb paths; using a leading hitbox-only destroy would free the body before bounce. No fifth physics layer needed.
