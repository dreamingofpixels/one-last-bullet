...
- **Decision**: Capturing an orb no longer pauses the tree or opens `RitualMenu.open()`. Circle shows world `%GlyphSlots` for the whole capture and `%OrbInfo` only while a player is inside a separate larger `%InfoProximityArea` (DepositArea stays smaller for activate / deposit). `%BuyBlankOrb` uses that same info radius (idle / waiting only; still hidden during capture). Each glyph socket uses a tiny **`socket_interact_radius`** (default **12 px**). **Pickup (pad L2)** within that radius of a socket **places** a carried glyph into that slot (if empty) or **removes** that slot's glyph into hands (if filled); DepositArea alone no longer auto-fills first/last slot. While `_ritual_running`, DepositArea **does not** suck glyphs to mana — socket L2 is the only glyph path. Glyphs entering DepositArea with **no** captured orb still convert to mana; **no glyph inventory**. **Activate (Square / E)** arms the circle for capture. **Upgrade (Triangle / F)** buys a blank orb (`live_orb_count × 20`, cap **5**) while idle or waiting, or with 3 glyphs during ritual: Transform if `OrbRecipes.is_playable` (then **launch** the new orb), else **bake**. **Ritual cancel (Circle / C)** releases the orb with sockets intact. Esc **inspect** still pauses and shows attribute `desc` from GameData sheet **`attribute`**. While `_circle_captured`, the orb is **fully inert**: no player/enemy damage, no status/knockback/splash, no Conduit current, no body/hitbox collision (`monitorable` off + collision layers cleared). After **exactly two** glyphs are socketed, `%FireHint` / `%WaterHint` / `%AirHint` / `%EarthHint` on the matching OrbInfo stat rows show the Transform name (or `???`) for adding that element third via `OrbRecipes.hints_for`. After **three** glyphs match a playable recipe, OrbInfo name / effect / stats preview that specialist (GameData row; socket icons stay). Hints hide at any other socket count. `OrbRecipes.is_playable` requires a loadable scene: authored `scene_path` if the file exists, else `res://entities/orbs/{id}/{id}_orb.tscn`.
- **Why**: Co-op flow — one player can ritual while the room keeps moving; show-don't-tell via live number deltas; bake lets non-recipe 3-glyph sets still upgrade stats; sockets stay readable at a distance while OrbInfo and the buy prompt only appear when close enough to use them. Per-socket L2 targeting makes sparse slots intentional (walk to the hole you want). Blocking mana suck during capture prevents accidental recycle when walking a glyph onto a socket. Splitting activate onto Square lets Triangle buy blanks without arming the circle first. Cap **5** leaves room after the 3-orb opening volley. Inert capture keeps the parked orb from punishing players who stand in the circle or from passively farming enemies mid-ritual. Element-row hints teach the third glyph without menu focus or stealing movement. A 3-glyph OrbInfo preview is the confirm beat; Triangle then launches so craft does not need a second Circle press.
