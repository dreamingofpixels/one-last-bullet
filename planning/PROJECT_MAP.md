...
- `project/components/hitbox_component.gd` — `Area2D`; physics-frame overlap poll of attacker `HitboxComponent`s; frame dedup via `hit_dedup_frames`; contact ticks from `DamageComponent.contact_damage_interval`; skips if instigator == owner; skips attackers in group `orb` (orb damage is body-contact / tether probe); `set_invulnerable(bool)` clears overlap state and toggles monitoring (deferred)
