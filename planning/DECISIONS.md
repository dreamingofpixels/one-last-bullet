...
- **Decision**: Destructible cliff segments are a dedicated `Cliff` breakable (`cliff.tscn` / `cliff.gd`). Authors pick `kind` among eight sheets — `corner_SW` / `corner_SE` / `wall_1` / `wall_2` / `corner_NW` / `corner_NE` / `vertical_wall_W` / `vertical_wall_E` — and may preview `frame_index` in the inspector; an `@tool` apply sets the vertical-strip `Sprite2D` (`hframes` 1 / `vframes` 4; vertical walls are 16×32 cells, others 32×32) plus body/hitbox rects. Does **not** call `LevelObject._ready()` / random `LevelObjectVariant`. Default HP **80**. Optional collision size/offset overrides. **At runtime**, `frame_index` is overwritten from 32 px neighbors (group `cliffs`; left / right / above / below, 4 px slop; origins stay on the 32 px grid even for 16 px-wide verticals). `Cliff.refresh_all_frames` runs deferred on ready and when any cliff is destroyed (dying cliff leaves `cliffs` immediately and is skipped if queued for deletion). Frame rules: SW/NW corners map both-connected → 0, either relevant side → 1, neither → 3; SE/NE distinguish the two single-neighbor cases as 1 vs 2; horizontals use left+right (0–3); verticals use above+below (0–3).
- **Why**: Kind is placement authorship; frame must match living neighbors so destroying a segment reveals the correct cut faces without hand-wiring each instance. Editor frame remains for layout preview before play. Appending new enum values keeps existing desert `kind = 0..3` instances stable.
- **Alternatives**: Manual `frame_index` only — drifts as soon as anything breaks; TileMap terrain peering — cannot carry per-piece HP / destroy VFX; random `LevelObjectVariant` — wrong for authored cliff shapes; 16 px adjacency cell for verticals — rejected, placement stays on the shared 32 px origin grid.
- **Status**: decided (in-codebase)
