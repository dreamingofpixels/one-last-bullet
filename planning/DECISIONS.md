...
- **Decision**: Capturing an orb no longer pauses the tree or opens `RitualMenu.open()`. Circle shows world `%GlyphSlots` for the whole capture and `%OrbInfo` only while a player is inside a separate larger `%InfoProximityArea` (DepositArea stays smaller for activate / deposit). Each glyph socket uses a tiny **`socket_interact_radius`** (default **12 px**). **Pickup (pad X / Cross)** within that radius of a socket **places** a carried glyph into that slot (if empty) or **removes** that slot's glyph into hands (if filled); DepositArea alone no longer auto-fills first/last slot. While `_ritual_running`, DepositArea **does not** suck glyphs to mana — socket Cross is the only glyph path. Glyphs entering DepositArea with **no** captured orb still convert to mana; **no glyph inventory**. **Activate (Square / E)** arms the circle for capture. **Upgrade (Triangle / F)** buys a blank orb (`live_orb_count × 20`, cap **5**) while idle or waiting, or with 3 glyphs during ritual: Transform if `OrbRecipes.is_playable`, else **bake**. **Ritual cancel (Circle / C)** releases the orb with sockets intact. Esc **inspect** still pauses and shows attribute `desc` from GameData sheet **`attribute`**. While `_circle_captured`, the orb is **fully inert**: no player/enemy damage, no status/knockback/splash, no Conduit current, no body/hitbox collision (`monitorable` off + collision layers cleared).
- **Why**: Co-op flow — one player can ritual while the room keeps moving; show-don't-tell via live number deltas; bake lets non-recipe 3-glyph sets still upgrade stats; sockets stay readable at a distance while OrbInfo only appears when close enough to use it. Per-socket Cross targeting makes sparse slots intentional (walk to the hole you want). Blocking mana suck during capture prevents accidental recycle when walking a glyph onto a socket. Splitting activate onto Square lets Triangle buy blanks without arming the circle first. Cap **5** leaves room after the 3-orb opening volley. Inert capture keeps the parked orb from punishing players who stand in the circle or from passively farming enemies mid-ritual.
- **Alternatives**: Keep paused full menu — bad for 2P; ward around the circle — deferred for later feel tests; world-socket onto flying orbs — rejected for accidental craft; require recipe for any 3-glyph commit — rejected (bake covers attribute-only upgrades); always-on OrbInfo while captured — cluttered when fighting away from the circle; reuse DepositArea for OrbInfo proximity — too tight for reading stats; only disable orb `monitoring` — insufficient because victim hitboxes still detect a `monitorable` attacker Area2D; first-empty / last-filled Cross socketing — previous, could not target a specific gap; allow mana deposit during capture — previous bug, stole glyphs meant for sockets; Triangle for both activate and buy — previous, blocked buy while idle and collided with activation; cap **3** — previous, made buy impossible with the 3-orb opening.
- **Status**: decided (in-codebase)
