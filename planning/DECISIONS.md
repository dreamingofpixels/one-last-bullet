...
- **Decision**: Mid-combat steer is **dash into a flying orb**. While dashing, each physics step tests the dash segment against orbs within `vault_catch_radius` (**28 px**). On catch: end dash immediately via `DashComponent.end_and_clear_cooldown()` (refunds CD), snap the player to the far side along the dash (`orb + dash_dir * (orb_radius + body_radius + gap)`), then `CollisionSeparation.separate` against world|wall. Orb enters vault hold while staying `FLYING`: `freeze`, zero velocity, trail off, instigator + **1 s** grace. Chevron follows aim for up to **`vault_hold_duration` (0.5 s)**; Attack fires early (consumes attack CD + attack visual) or timeout auto-fires (no attack CD). Fire calls `deflect()` (**+10%** boost, unfreeze, grace refresh). One vault at a time; other orbs ignored until fire. Gated by `OrbTetherComponent.dash_vault_enabled` (**true** on player). Proximity Attack redirect stays in code but inactive while the flag is on.
- **Why**: Makes batting a committed intercept instead of loitering in a 48 px bubble; dash becomes the bat; freeze-only-that-orb avoids global slow-mo co-op issues; aim-during-hold keeps mouse/stick useful; dash refund preserves the dodge-out after a bat.
- **Alternatives**: Keep proximity Attack redirect — previous playtest default, weaker commitment; lock chevron to dash facing only — drops aim agency; keep dash cooldown after vault — leaves the player stranded next to a stopped hazard; delete proximity redirect — harder to A/B; movement lock during hold — too punishing with packs.
- **Status**: decided (in-codebase); revisit after playtest
