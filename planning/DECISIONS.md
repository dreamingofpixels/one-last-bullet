...
- **Decision**: `AttackComponent.melee_enabled = false` on the player. Idle/Walk Attack no longer enters the Attack state after a failed redirect. Swing sprite, hint, hitbox, and enemy knockback stay in the scene/script but do nothing. Attack input is further parked (no Idle/Walk Attack handling); glyph throw moved to **pickup**; vault early-fire moved to **dash-again**. Flip `melee_enabled` to restore melee when Attack is re-enabled.
- **Why**: Playtesting steer-only combat without deleting the arc system; knockback was fighting orb-focused combat feel; parking Attack entirely keeps the button free for a future verb.
- **Alternatives**: Delete Attack state / KnockbackComponent — harder to restore; keep out-of-range swing — mixed fantasy while redirect is the only useful Attack verb; keep throw on Attack — previous.
- **Status**: decided (in-codebase); revisit after playtest
...
- **Tether feel**: playtest mid-combat steer is **dash vault** (`dash_vault_enabled`); proximity Attack redirect parked behind the same flag. Orbit capture remains parked (`capture_enabled`); entity punch-through is the default (desert export can enable bounce). Vault early-fire is **dash-again** (spends dash); timeout refunds dash CD.
- **Attack cooldown / charges**: Attack input parked; dash-again early vault fire starts dash CD; timeout auto-fire refunds dash CD; tether post-release cooldown 0.25s; melee parked.
