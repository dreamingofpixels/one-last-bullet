...
- `project/entities/orbs/vulcano/vulcano_orb.tscn` + `vulcano_orb.gd` (`class_name VulcanoOrb`) — volcanic art; opts out of constant-speed flight (`_keeps_constant_flight_speed`); slides with damp then parks; contact damage/burn from GameData; while parked spews `lava_globe.tscn` into `%WorldYSort`
- `project/entities/orbs/vulcano/lava_globe.tscn` + `lava_globe.gd` (`class_name LavaGlobe`) — fly (4-frame) → splat (8-frame) → puddle (4-frame loop, 5 s); puddle Area2D applies 1 Burn/s to enemies; source = owning Vulcano
...
- `project/objects/_base/breakable.gd` — `extends LevelObject`; `COMPONENTS` dict; `breakables` group; destroyed via `COMPONENTS[HealthComponent].take_damage()` from orb `_try_apply_orb_damage` on `body_entered` or enemy melee `HitboxComponent` overlap
...
- `project/items/glyphs/glyph.tscn` + `.gd` — RigidBody2D pickup on `item` layer; `@tool` **`glyph_id`** inspector enum from GameData; **Rarity**; carry at player origin (`CARRY_OFFSET` zero); deposit → circle mana (when no ritual); group **`glyphs`**
