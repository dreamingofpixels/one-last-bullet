...
- **Alternatives**: Instant pull regardless of distance — removes the dodge/positioning challenge; pull orb gradually toward player over 2 s — more complex physics (orb still bouncing); complete channel early if orb enters range mid-hold — conflates tap and hold UX; no movement lock during channel — too safe, trivializes orb retrieval.
- **Status**: decided (in-codebase)
