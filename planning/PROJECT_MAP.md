...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene), `bind_orb()` / `begin_opening_tether()`; capture/release via `try_tether_press()` on the tether action; short post-release cooldown; **remote tether channel**: hold tether 2 s out of range → snaps orb to `focus_radius` and captures; `is_channeling()` / `get_channel_progress()`; draws progress ring + line to orb; input centralized in `_process()` (player states only gate movement/dash/attack)
