...
- **Decision (design)**: Socketing glyphs boosts one of the orb's 12 attributes (Common / Rare / Unique values). **Three glyphs on a Blank Orb** can upgrade it into one of **20** specialist orbs with special effects. **Three glyphs on a non-Blank orb** can, with the right combination, trigger **Attunement**. After **two** glyphs are socketed, `%OrbNameLabel` hides and the live circle shows only Transform hints in `%HintContainer` (2×2 grid; recipe **name** if discovered / row exists and GameData `active` is not `false`, `???` otherwise). `%OrbNameLabel` is visible at **0 / 1 / 3** glyphs. `%HintArrow` stays in the scene but hidden. Each Name-row label is **modulated** by the missing third element's glyph fill (Fire `#a53030` / Water `#4f8fba` / Air `#c7cfcc` / Earth `#ad7757`). Attribute-row hints stay in the scene but hidden. After **three** glyphs match a playable recipe, OrbInfo previews that specialist (name / effect / GameData stats); Triangle confirms Transform and **launches** the new orb. Two-glyph hints do not swap the whole panel.
- **Why**: Gives Blank Orbs a craft identity and specialist orbs a further chase; the two-glyph hint teaches recipes without inventing names for missing data; the Name-row grid plus element tint shows which glyph to add next without stealing walk input or crowding the stat rows; a full 3-glyph preview is the confirm step before the orb flies out.
- **Alternatives**: Glyphs only ever flat-add stats — no orb identity change; auto-upgrade on any three glyphs without recipes — less discovery; show all recipe names immediately without a discovery seam — harder to add meta later; menu hover/focus preview + 3-glyph single hint — previous paused-menu UX, superseded for live ritual; keep OrbInfo on the Blank after three glyphs — rejected, hid the craft result until after Triangle; Transform in-place and keep captured — previous, extra Circle press to launch; element-row hints alongside Name-row — previous live UX, hidden for now (element modulate on the Name-row is enough).
- **Status**: decided (in-codebase) for Blank→specialist Transform when a scene resolves (Ghost/Contagion/Conduit/Vulcano/Runic/Nova); Attunement Transform not playable yet (no scenes); full 20-orb table still incomplete; live Name-row hints (names + element modulate) plus 3-glyph OrbInfo preview in-codebase
...
- **Status**: superseded — live circle ritual; buy cost is now `live_orb_count × 20`; glyph inventory removed

### Ritual menu v2: drag socketing, Transform, always-on discovery
- **Decision**: Replaced the button-row ritual UI with `ritual_menu.tscn` + bottom `OrbInventoryBar`. Glyphs move via **mouse hold-drag** or **controller** (d-pad or left-stick flick navigates focus across inventory glyphs, orb slots, recycle, Transform, **hints**, Buy, Done — analog is **one step per flick**, then the stick must recenter; **bottom orb sockets are not focusable**; dash grabs / confirms; **pickup** cancels). Orb glyph slots are **sparse** (fixed 3 indices; gaps allowed). Mouse drop onto an occupied slot **swaps** on release. Controller: moving a held glyph onto an occupied orb slot **live-swaps** (the other glyph jumps into the empty hole left behind — orb pickup hole, or the empty slot auto-targeted from inventory; you keep holding the same glyph). Inventory-held entering the orb row **auto-targets the first empty slot** when one exists; if the orb is full, confirm still swaps displaced back to inventory (no live-swap). **Focus** = white outline; **held** = darkened floating preview. Confirm on a socketed glyph **picks it back up** instead of auto-returning it to inventory. Focus stays on the circle-captured orb for editing. Dragging hides the source socket icon. Starting circle glyphs / mana / opening orbs come from **`RunStartConfig`**. After **2** socketed glyphs, four hints appear from `OrbRecipes`; after **3**, a **single centered larger** result hint appears when `result_for` is non-empty (hidden otherwise). **Controller focus or mouse hover** on a hint previews that recipe’s name / effect / attributes in the main orb panel (`???` → placeholders; leave restores the captured orb). Hint labels and the top orb name are **uppercase**. No side info panel on Transform hover. **Transform** enables only when 3 glyphs match a row with a non-null `scene_path`. Discovery is provisionally **always on**.
- **Why**: Matches the authored v2 layout; one drag model for mouse and pad; independent slots match visual drop targets; live-swap makes pad rearrange feel immediate while keeping the held glyph (from orb or inventory); darkened held preview reads apart from focus; inspectable hints teach recipes in the main panel without a separate tooltip; the 3-glyph single hint confirms the locked combo; Transform gated on playable scenes avoids spawning broken orbs; discovery seam keeps meta-unlock work deferred.
...
- **Decision**: Level start launches **one Ghost**, **one Contagion**, and **one Conduit** orb from the summoning circle in independent random directions (`begin_flight`, player grace). No plain blank orb is spawned. `blank_orb.gd` / `.tscn` remain the shared `BlankOrb` base (`class_name`, `FLYING` / `TETHERED` / `POSSESSED`). Typed scenes use per-orb art and override hit hooks where needed (Ghost possession; Contagion Disease roll). Each orb loads **12 core stats** from GameData via `orb_id`. Enemies own a `StatusComponent` for Blight, Shock, Burn, Chill, and Disease. Ghost follows its host in world space (not reparented) so enemy `queue_free` cannot free the orb. Conduit current is a `Line2D` + enemy-mask capsule `Area2D` while the closest player is within 130 px.
...
- **Decision**: Orbs live under `project/entities/orbs/` with one subfolder per type: `blank/` (shared `BlankOrb` base), `ghost/`, `contagion/`, `conduit/`, `vulcano/`, `runic/`. Shared SFX in `orb_sfx/`. Typed `class_name`s are `GhostOrb`, `ContagionOrb`, `ConduitOrb`, `VulcanoOrb`, `RunicOrb` (all extend `BlankOrb`). The old `ChaosOrb` class name and `entities/chaos_orb/` path are retired; `RotOrb` / `rot/` are renamed to Contagion.
- **Why**: Clearer per-type organization as more orb variants are added; `BlankOrb` matches the neutral base scene; Ghost/Contagion/Conduit/Vulcano/Runic names match the current design vocabulary.
...
### Contagion Disease status (half-blight spread on death)
- **Decision**: Contagion's special effect is a **20%** chance on enemy hit to apply **Disease** (permanent flag until death; sickly-green sprite tint via `HealthComponent.rest_modulate`). Blight still comes from the orb's GameData `blight` attribute (currently **3**) through the shared `BlankOrb` status path — Disease is additive, not a replacement for blight application. On death of a Diseased enemy, every other enemy within **100 px** gains `floor(blight_stacks / 2)` Blight stacks. Recipients do **not** become Diseased (no chain reaction). Spread uses the same shape-query helper as nearby-enemy status AOE.
- **Why**: Moves Contagion past "just applies blight" into a pack-contagion fantasy while keeping blight readable as a base stat; no chaining keeps room clears from runaway Disease cascades; a rest-modulate tint survives damage flashes so Disease stays readable.
...
- **Alternatives**: Keep Burn as death explosion — left Fire without a DoT and Earth with Poison; expire after a wall-clock duration — another clock besides the tick; remove 1 stack per tick — weaker investment; amp Burn ticks with Blight — rejected (orb-only amp; see Blight).
- **Status**: decided (in-codebase)

### Blight: 5% dealt / 5% orb taken, cap 90%
...
- **Decision**: Vulcano (`Fire`+`Earth`+`Earth`) stays `FLYING` for vault/capture but opts out of BlankOrb constant-speed flight via `_keeps_constant_flight_speed()`. After launch or vault redirect it slides with glyph-like damp (`linear_damp` 5, stop ≤ 12 px/s), then freezes as a parked turret: **hitbox monitoring and monitorable off** while parked (victims poll attacker areas — monitoring alone is not enough; no contact damage/status; puddles own Burn), but **collision_layer becomes world|orb** so player/enemy `move_and_slide` treat it like a prop (orb-only layer punches through entities). While parked it spews lava globes to a random 28–72 px ring, retrying landings so they stay ~22 px clear of other active globes/puddles and avoid world-layer props/walls (skip spew if no spot); each globe arcs, plays splat, then a puddle applies **1 Burn/s** to enemies (source = Vulcano for glyph-drop). Flying globes parent to `%WorldYSort` and y-sort along the ground track; on splat they reparent to `%GroundEffects` so puddles stay under entities and orbs. Spew pauses while vault-held or circle-captured; existing globes/puddles keep running. Feel constants live on the Vulcano scripts, not a new GameData sheet. Scene at `entities/orbs/vulcano/vulcano_orb.tscn` (OrbRecipes fallback; authored JSON path under `blank/` is unused).
...
### HomingComponent (orb heading steer)
- **Decision**: Shared `HomingComponent` steers a caller-supplied heading toward a target. Exports `turn_rate` (rad/s) and `use_pathing` (navmesh path via level `%Navigation`, else straight seek). API: `set_enabled` / `set_target_node` / `set_target_position` / `clear_target` / `steer(current_dir, delta)`. Does **not** pick targets, move the body, or change collision — the orb applies the result to `BlankOrb.aim_direction` after `super._physics_process` so bounce wins that frame. Not a reuse of enemy `NavigationComponent` (CharacterBody2D + MovementComponent).
- **Why**: Multiple future orbs need imperfect homing with optional obstacle pathing; keeping target selection and special effects on the orb avoids a one-size AI.
- **Alternatives**: Inline steer/path in each specialist — duplicates turn-rate and nav helpers; extend `NavigationComponent` for orbs — wrong body type and couples chase AI; perfect aim rewrite of `aim_direction` each frame — too sticky and fights bounce feel.
- **Status**: decided (in-codebase)

### Runic glyph fetch and drop ring
- **Decision**: Runic (`Water`+`Water`+`Water`) uses `HomingComponent` (`use_pathing` true, turn rate ~2.4) to seek pickable floor glyphs, skipping ones already in a ~28–48 px ring around the summoning circle. Hitbox overlap picks up without HP damage (`should_apply_hitbox_damage` false for glyphs); carried glyphs clear PresenceArea so DepositArea cannot mana-suck them mid-flight. One glyph at a time; drop settles on a ~34 px ring (outside DepositArea radius 22) via `Glyph.drop_at` (no throw velocity). Circle capture drops the carried glyph onto the ring first; vault keeps the glyph. Flying collision mask always includes **enemy** so Runic bounces off enemies then resumes pathing; player punch-through stays unless `bounce_off_entities`. Idle bounce when no glyphs to fetch. Scene at `entities/orbs/runic/runic_orb.tscn` (OrbRecipes fallback; authored JSON path under `blank/` unused). Attunements Enchant / Hoarder / Secure / Affinity stay inactive.
- **Why**: Matches the glyph-courier fantasy without auto-depositing mana; imperfect steering keeps room chaos; enemy bounce matches the design note without enabling player bounce.
- **Alternatives**: Auto-deposit into the circle — steals player agency and mana timing; perfect lock-on — too strong and clips less; reuse player throw for drop — overshoots into DepositArea; steal player-carried glyphs — fights co-op pickup.
- **Status**: decided (in-codebase)

### Nova frost-nova pulse
- **Decision**: Nova (`Water`+`Water`+`Air`) stays a normal flying `BlankOrb`. While `is_flying()` it pulses every `nova_interval` (default **3 s**): spawn `frost_nova` as a **child of the orb** (follows motion), and apply `nova_chill` (default **5**) Chill stacks to enemies within `nova_radius` (default **48 px**). First pulse waits a full interval after ready. Cooldown **pauses** while circle-captured (inert); continues during vault hold. No pulse HP damage; player is never chilled. Pulse Chill is a scene export and is **not** tied to the orb's GameData `chill` on-hit stat (impact still applies that separately). Feel knobs live on the orb scene (`nova_radius` / `nova_interval` / `nova_chill` / `nova_anim_fps` / `nova_visual_scale` / optional `nova_sound`). Scene at `entities/orbs/nova/nova_orb.tscn` (OrbRecipes fallback; authored JSON path under `blank/` unused).
- **Why**: Matches the periodic AoE chill fantasy without inventing a new status or DoT; parenting FX to the orb keeps the ring readable on a moving projectile; separating pulse Chill from on-hit `chill` lets glyph socketing still matter for impacts without snowballing the pulse.
- **Alternatives**: Parent FX under `%WorldYSort` at pulse origin — leaves the ring behind as the orb flies; scale pulse Chill from GameData `chill` — mixes glyph bonuses into the special effect and fights the authored "5 Chill" text; deal pulse HP — duplicates splash/impact identity; tick only while parked like Vulcano — wrong motion fantasy for Nova.
- **Status**: decided (in-codebase)
...
- **Status**: superseded — Poison replaced by Blight; DoT moved to Burn (see above)
...
- **Decision**: Enemies (and, per source design, obstacles) roll **`glyph_drop`** from the killing orb (GameData `orbs` row; level fallback export). On success, spawn a **`Glyph`** (one of 12 ids from GameData `glyphs`, weighted **Common 70% / Rare 25% / Unique 5%**). Carry/throw/deposit flow: **pickup** in focus, **pickup again** throws, dash blocked while carried, **20 HP** orb-destroyable. Deposit into `SummoningCircle`: suck to center → **`receive_glyph`** stores `{id, rarity}` in a **3-slot** inventory; overflow credits mana (**5 / 10 / 20**). Each glyph uses its own PNG from GameData `glyphs.scene_path` (e.g. `sooth` → `sooth_glyph.png`); rarity shown by **unison center-rune blink** (Common white, Rare jade + rising motes, Unique gold + denser motes). `ManaCrystal` retired.
- **Why**: Glyphs are the upgrade vector for the ritual menu; mana comes from discard/overflow rather than every pickup.
- **Alternatives**: Keep crystals as mana and add separate glyph drops — two loot types to juggle; auto-socket on deposit — removes ritual menu choice; throw on Attack — previous, superseded while Attack is parked.
- **Status**: decided (in-codebase)
...
- **Status**: superseded — live circle ritual (no pause); see below

### Live circle ritual: no pause, world OrbInfo / GlyphSlots
- **Decision**: Capturing an orb no longer pauses the tree or opens `RitualMenu.open()`. Circle shows world `%GlyphSlots` for the whole capture and `%OrbInfo` only while a player is inside a separate larger `%InfoProximityArea` (DepositArea stays smaller for activate / deposit / buy). Device-aware `%PromptStack` (`InputPrompt` Buy / Activate / Cancel / Transform-or-Bake) uses that same info radius, with Buy and Activate prompts gated to DepositArea. Each glyph socket uses a tiny **`socket_interact_radius`** (default **16 px**). **Pickup (pad L2)** within that radius of a socket **places** a carried glyph into that slot (if empty) or **removes** that slot's glyph into hands (if filled); DepositArea alone no longer auto-fills first/last slot. While `_ritual_running`, DepositArea **does not** suck glyphs to mana — socket L2 is the only glyph path. Glyphs entering DepositArea with **no** captured orb still convert to mana; **no glyph inventory**. **Activate (Triangle / Space)** arms the circle for capture (DepositArea) and, with 3 glyphs during ritual, Transform if `OrbRecipes.is_playable` (then **launch** the new orb) else **bake** then **launch** (accepted from InfoProximityArea so sockets work). **Upgrade (Square / E)** buys a blank orb (`live_orb_count × 20`, cap **5**) from DepositArea (same range as activate) while idle or waiting. **Ritual cancel (Circle / Q)** from InfoProximityArea disarms a waiting circle (no mana refund) or releases a captured orb with sockets intact. World sockets sit outside DepositArea (~33 px vs radius 22), so Transform/cancel poll InfoProximityArea; activate and buy stay DepositArea. Esc **inspect** still pauses and shows attribute `desc` from GameData sheet **`attribute`**. While `_circle_captured`, the orb is **fully inert**: no player/enemy damage, no status/knockback/splash, no Conduit current, no body/hitbox collision (`monitorable` off + collision layers cleared). After **exactly two** glyphs are socketed, `%OrbNameLabel` hides and `%HintContainer` (`%Hint1`–`%Hint4`) shows the Transform name (or `???`) for adding that element third via `OrbRecipes.hints_for`, with each label **modulated** to the missing element's glyph fill (Fire `#a53030` / Water `#4f8fba` / Air `#c7cfcc` / Earth `#ad7757`). `%HintArrow` stays in the scene but hidden. `%FireHint` / `%WaterHint` / `%AirHint` / `%EarthHint` on the stat rows stay in the scene but hidden. After **three** glyphs match a playable recipe, OrbInfo name / effect / stats preview that specialist (GameData row; socket icons stay). Hints hide at any other socket count (`%OrbNameLabel` visible at 0 / 1 / 3). `OrbRecipes.is_playable` requires a loadable scene: authored `scene_path` if the file exists, else `res://entities/orbs/{id}/{id}_orb.tscn`. Release grants a brief capture grace so a launched orb is not immediately recaptured.
- **Why**: Co-op flow — one player can ritual while the room keeps moving; show-don't-tell via live number deltas; bake lets non-recipe 3-glyph sets still upgrade stats, then launches so craft never needs a second Circle press (sockets sit outside DepositArea, so Triangle must read InfoProximityArea); sockets stay readable at a distance while OrbInfo only appears when close enough to read; buy uses the same DepositArea as activate so you cannot spawn an orb from the info ring. Per-socket L2 targeting makes sparse slots intentional (walk to the hole you want). Blocking mana suck during capture prevents accidental recycle when walking a glyph onto a socket. Square buys blanks without arming; Triangle starts the ritual and commits Transform/bake (see face-button entry). Cap **5** leaves room after the 3-orb opening volley. Inert capture keeps the parked orb from punishing players who stand in the circle or from passively farming enemies mid-ritual. Name-row HintContainer (element-tinted labels; name hidden at two glyphs) teaches the third glyph without menu focus or stealing movement. A 3-glyph OrbInfo preview is the confirm beat; Triangle then launches.
- **Alternatives**: Keep paused full menu — bad for 2P; ward around the circle — deferred for later feel tests; world-socket onto flying orbs — rejected for accidental craft; require recipe for any 3-glyph commit — rejected (bake covers attribute-only upgrades); always-on OrbInfo while captured — cluttered when fighting away from the circle; always-on Buy Blank Orb label — previous, cluttered the arena when no one was near the circle; buy from InfoProximityArea — previous, wider than activate so Square/E could spawn an orb before you were on the circle; reuse DepositArea for OrbInfo proximity — too tight for reading stats; only disable orb `monitoring` — insufficient because victim hitboxes still detect a `monitorable` attacker Area2D; first-empty / last-filled Cross socketing — previous, could not target a specific gap; allow mana deposit during capture — previous bug, stole glyphs meant for sockets; Triangle for both activate and buy — previous, blocked buy while idle and collided with activation; cap **3** — previous, made buy impossible with the 3-orb opening; menu HINT focus / hover recipe preview — superseded for live ritual (stick is walk, not menu nav); keep OrbInfo on the Blank after three glyphs / Transform in-place without launching — previous, hid the result and needed Circle to let the orb loose; element-row hints beside the stats — previous, hidden for now (Name-row + element modulate is enough).
- **Status**: decided (in-codebase)

### Ritual face buttons: Triangle start/commit, Circle stop, Square buy
- **Decision**: Gamepad **Triangle / F** (`activate`) arms the circle from DepositArea and, with 3 socketed glyphs, Transform/bake then launch from InfoProximityArea. **Circle / C** (`ritual_cancel`) always means stop: disarms a waiting (armed, no orb) circle with **no mana refund**, or releases a captured orb with glyphs intact. **Square / E** (`upgrade`) buys a Blank Orb from DepositArea (same range as activate) while idle or waiting. Triangle does nothing with 0–2 glyphs (no silent cancel). Keyboard follows the same split.
- **Why**: Confirm vs cancel stay on different face buttons, so the button that starts the ritual is also the one that finishes a ready craft — not the one that dumps a live orb. Buy stays off Triangle so idle activate and idle buy cannot collide. Waiting-cancel was missing; Circle is the existing abort verb, so it also shuts the circle off before an orb arrives.
- **Alternatives**: Square activate / Triangle buy+transform — previous; Triangle toggle on/off with Square Transform — rejected, because the start button would dump the orb at the 3-glyph climax; Triangle activate+buy — previous, blocked buy while idle; buy from InfoProximityArea — previous, wider than activate; refund mana on waiting-cancel — rejected for now (cancel is “wrong time,” not a free probe).
- **Status**: decided (in-codebase)

### Glyph stat upgrades: flat-add from GameData `attribute`
- **Decision**: Each `glyphs` row names an **`attribute`** (`damage`, `burn`, `glyph_drop`, etc.). Socketing adds **`rarity_common` / `rarity_rare` / `rarity_unique`** flat to that stat. `splash`, `crit_chance`, `glyph_drop` clamp to **0–1**; `damage`, `self_damage`, `speed`, `weight` floor at **0**. Negative values in data (e.g. `sooth`, `drag`) reduce stats as authored. Unknown attributes fail the socket (L2 then throws). Legacy **`poison`** is accepted as **`blight`** so Spore still sockets if a sheet export lags behind the Poison→Blight rename.
...
- **Decision**: `AttackComponent.melee_enabled = false` on the player. Idle/Walk Attack no longer enter the Attack state. Swing sprite, hint, hitbox, and enemy knockback stay in the scene/script but do nothing. Attack input (**F / R1**) drives the orb bat via `try_attack_bat()` without entering the Attack state; glyph throw stays on **pickup**; vault early-fire stays **dash-again**. Flip `melee_enabled` to restore melee swing on the same Attack binding.
- **Why**: Playtesting steer-only combat without deleting the arc system; knockback was fighting orb-focused combat feel.
- **Alternatives**: Delete Attack state / KnockbackComponent — harder to restore; keep out-of-range swing — mixed fantasy while redirect is the useful Attack verb; keep throw on Attack — previous.
- **Status**: decided (in-codebase); revisit after playtest
...
- **Status**: superseded — replaced by movement-cone Attack bat (see below); legacy `try_redirect_attack` path kept for vault early-fire / non-vault fallback

### Attack bat (180° move-facing cone; hold for 360°)
- **Decision**: Mid-combat Attack is **R1 / F** (`attack` / `attack_2`) when `orb_redirect_mode == ATTACK_BAT` (**dwarf**). Press begins a charge hold on frame 0 of `attack_around`. Release under **`attack_charge_hold_seconds` (0.5 s)** plays body `attacking` and bats **every** flying orb in a **180° half-circle** in front within `focus_radius` (**48 px**). Hold ≥ 0.5 s then release plays `attack_around` and bats every flying orb in **360°** within focus radius. Forward (cone) = live move / left stick; if standing still, `DirectionalSpriteComponent.facing_vector()`. Launch direction is `Controls.get_explicit_aim_vector()` (else cone forward). Each hit calls `BlankOrb.deflect()` (**+10%** speed/damage). Shares **0.35 s** attack cooldown via `consume_cooldown()`. Does **not** reset dash cooldown. Body clip plays even with no orbs in range; dwarf skips `AttackComponent.play_swing_visual()` (hammer art is the swing). Chevrons show on the **closest** cone-eligible orb **only while explicitly aiming**. Blocked / charge cancelled while assembling, dashing, vault-aiming, tethering, channeling, or carrying a glyph. Mutually exclusive with dash vault via `OrbTetherComponent.orb_redirect_mode` (set from player `character`).
- **Why**: Gives a walk-up bat that reads as a front half-swing; hold-to-spin sells the dwarf hammer fantasy and a full-circle clear without a second binding; body clips replace the wizard arc for the dwarf.
- **Alternatives**: Instant tap-only bat — previous; always-on chevron when in range — previous, noisy on pad; refund dash on bat — previous proximity redirect; run bat and vault together — rejected (character pick instead); enable full `melee_enabled` for sprite — would restore knockback.
- **Status**: decided (in-codebase)

### Orb redirect mode (per character, not level A/B)
- **Decision**: `OrbTetherComponent.OrbRedirectMode` enum (`ATTACK_BAT` / `DASH_VAULT`) remains the mid-combat steer switch. `dash_vault_enabled` / `attack_bat_enabled` are read-only getters. **Player `character`** sets the mode in `_apply_character()`: **Wizard → Dash Vault**, **Dwarf → Attack Bat**. Level / desert no longer export or overwrite `orb_redirect_mode`.
- **Why**: Redirect fantasy is tied to the body you play; inspector character pick is enough for playtest without a second level-wide knob fighting the player scene.
- **Alternatives**: Desert `orb_redirect_mode` applied to all players — previous A/B toggle, superseded; two independent bools — allowed both on; runtime HUD toggle — deferred.
- **Status**: decided (in-codebase); supersedes “Orb redirect mode playtest toggle”

### Player character select (wizard / dwarf)
- **Decision**: `player.gd` exports `CharacterId` (`WIZARD` / `DWARF`). Choosing **Dwarf** swaps `%PlayerSprite` to `dwarf/dwarf_frames.tres` (64×32 SW-only: idle 1–5, run 6–14, attack_front 21–26 → `attacking_sw`, attack_around 27–28) and sets Attack Bat. Choosing **Wizard** keeps `player_frames.tres` (SW+NW) and Dash Vault. `DirectionalSpriteComponent` falls back to `_sw` when `_nw` is missing and treats a pause-hold charge pose as still playing that action. Idle/Walk use `Player.is_attack_busy()` so charge / spin clips are not overwritten.
- **Why**: Second playable body with a distinct redirect fantasy without a second player scene; SW-only art works today via flip + fallback until NW exists.
- **Alternatives**: Separate `dwarf_player.tscn` — more duplication; level-wide character pick — deferred; force NW placeholders from SW copies in the frames resource — fallback in code is simpler.
- **Status**: decided (in-codebase)

### Dash vault redirect (playtest)
- **Decision**: When `orb_redirect_mode == DASH_VAULT` (**wizard**), mid-combat steer is **dash into a flying orb**. While dashing, `prepare_vault_dash` tests the **remaining dash ray** against orbs within `vault_catch_radius` (**28 px**) and **commits** to the soonest orb, clamping remaining distance to the far-side landing (`orb + dash_dir * (orb_radius + body_radius + gap)`). The dash **keeps moving** (afterimages, i-frames) until the player reaches that landing; then `try_finish_vault_dash` calls `end_and_clear_cooldown()` and starts the aim window (light place + `CollisionSeparation` only to settle, not a mid-dash teleport). Orb enters vault hold while staying `FLYING`: `freeze`, zero velocity, trail off, instigator + **1 s** grace. While vaulting, **player movement and dash are locked**. Chevron follows aim for up to **`vault_hold_duration` (0.5 s)** in **360°** (`vault_aim_half_angle_degrees` default **180**). **Press dash again** during the window to fire early along aim and `force_state("dash")` into a real dash (starts the 1.5 s cooldown). Dash-away direction prefers **WASD / left stick** when held (faces that vector before the Dash state starts); with no move input, falls back to last facing (usually the inbound vault dash). That same orb is ignored for vault catch for **`vault_recatch_cooldown` (0.35 s)** so dash-away does not immediately re-grab it (other orbs still catchable). Timeout auto-fires without a follow-up dash and **refunds** dash cooldown. Fire calls `deflect()` (**+10%** boost). One vault at a time. Attack bat is off in this mode.
- **Why**: Dash should read as a continuous bat through the orb, not a blink; aborting only once behind the orb sells the intercept; locking move/dash during the hold matches tether commitment; full-circle aim lets the player send the orb back through themselves (still covered by post-redirect grace) or anywhere else in the window. Dash-again lets the player cut the aim window short and immediately reposition at the cost of spending dash. Reading move input for dash-away (while still locking walk during the hold) lets the player leave sideways/back without the inbound facing forcing them through the orb again. Per-orb recatch grace stops the follow-up dash from re-vaulting the orb still on the ray.
- **Alternatives**: Instant snap to far side on ray hit — previous, felt like a teleport; catch only this-frame segment — felt like a full dash then freeze; free movement during hold — weaker commitment; **180°** cone along player→orb (`vault_aim_half_angle_degrees` 90) — previous, kept aim “away from me” but blocked reverse shots; Attack early-fire — previous, superseded by dash-again; proximity Attack redirect only — superseded by Attack bat mode; run bat and vault together — rejected for character clarity; block all vault catches during any dash after release — too harsh with multi-orb rooms; always dash along inbound facing on dash-away — previous, fought leave-direction intent.
- **Status**: decided (in-codebase); revisit after playtest

### Pickup input rename; Attack parked; glyph throw on pickup
- **Decision**: The former `tether` input action is renamed **`pickup`**. Dash is on **Space / R2**. **Attack** (LMB) is parked. Live ritual: pad **X → pickup**, **`activate`** (Square / E), **`upgrade`** (Y/Triangle / F), **`ritual_cancel`** (B/Circle / C). Glyph throw stays on pickup; Attack remains parked.
- **Why**: Separates loot/circle/ritual verbs from combat Attack; pad X for pickup matches the live ritual socket verb; Square activate vs Triangle buy avoids one button doing both.
- **Alternatives**: Keep throw on Attack — previous; keep Y as pickup — previous, remapped for live ritual; Triangle for activate+buy — previous, see live circle ritual entry.
- **Status**: superseded — pickup remapped to L2 (see below)

### Glyph pickup on L2; throw along right stick then move
- **Decision**: Gamepad **`pickup`** is **L2** (JoyAxis 4; devices 0/1), matching R2 for dash. Keyboard stays **right click**. **X / Cross** is unused. Same `pickup` press still grabs glyphs, throws if carrying, and places/removes ritual sockets. Throw direction uses `Controls.get_throw_aim_vector()`: **live right stick** if past deadzone, else **current move** (left stick / WASD), else player facing (then `Vector2.RIGHT`). No stick latch and no mouse fallback on throw. Vault/redirect keep `get_aim_vector` (P1 mouse fallback).
- **Why**: Free Cross for later; shoulder grab/throw mirrors dash on R2; throw should follow explicit aim or movement, not a lingering mouse cursor when playing on pad.
- **Alternatives**: Keep Cross for pickup — previous; latch last stick aim before throw — rejected (idle stick should mean move dir); mouse fallback on throw for P1 — previous, fights controller play; separate throw action — extra binding for little gain.
- **Status**: decided (in-codebase)

### Attack input removed
- **Decision**: There is no `attack` / `attack_2` input action and no `%AttackAction` on Controls. Left click is **dash**. Idle/Walk do not poll an attack press. `AttackComponent` + the Attack state stay in the player scene behind `melee_enabled` (false); proximity redirect stays behind `dash_vault_enabled`. Restoring melee needs a new binding as well as flipping the flag.
- **Why**: Dash vault and L2 pickup/throw cover combat; a parked attack action still occupied left click and implied a player swing that nothing read.
- **Alternatives**: Keep a parked `attack` action on LMB — previous; bind melee to Cross — left unused on purpose.
- **Status**: superseded — Attack rebound to R1 / F for the movement-cone bat (see “Attack bat” above); melee still parked

### Attack rebound to R1 / F (orb bat)
- **Decision**: `attack` / `attack_2` exist again. P1: **F** + gamepad **R1** (JoyButton 10, device 0). P2: R1 device 1. `%AttackAction` under Controls; `OrbTetherComponent` polls `is_attack_just_pressed()` in `_process`. Idle/Walk still do **not** enter the Attack state. Melee stays behind `melee_enabled` (false).
- **Why**: Dedicated shoulder/key for the front-cone bat without stealing LMB (dash) or L2 (pickup).
- **Alternatives**: Keep Attack removed — previous; bind to Cross — left unused; Left Shift / Ctrl / MMB — rejected for F in this playtest.
- **Status**: decided (in-codebase)
...
- **Status**: superseded — goblin and ogre both use committed `EnemyAttackComponent` melee (see below)

### Enemy committed melee via EnemyAttackComponent
- **Decision**: Enemies that own an attack clip use `EnemyAttackComponent` (Area2D): proximity enter → stop chase → play attack anim → **hold on an authored frame** → optional attack SFX → activate a **code-built rect hitbox** (editor `hitbox_size` / `hitbox_offset`) → finish anim → cooldown → resume chase. Optional **self-lunge** via `lunge_distance` > 0 (`KnockbackComponent.push_distance` on the attacker — goblin). Optional **victim knockback** via `hit_knockback_distance` > 0 (radial shove on the hit player's `KnockbackComponent` — ogre planted slam with large around-body box). Hitbox damage applies once per swing to the **player** and to any overlapping **breakables** (mask `player|world`; breakable hurtboxes are `HitboxComponent`s on the world layer with **mask 0** so they stay detectible but never poll orb `DamageComponent` hitboxes). Dash i-frames still skip via `monitoring == false`. Both goblin and ogre drop `DamageComponent` so passive contact no longer ticks. Chill slows via `speed_multiplier`; Shock stun calls `cancel()`. Spawner `set_active(false)` while assembling. Player Walk/Idle yield while knockback is active; dash cancels knockback. Orb weight knockback stays **enemies-only** so the player's slam-shove component does not bowl them on orb hits. Enemies still only **start** attacks when a player is in range (incidental prop smash, not AI targeting props).
- **Why**: Readable telegraphed threat that matches the attack art; one reusable component for lunging and planted AOE enemies; authored box size without sharing a mutated `RectangleShape2D` sub-resource across instances; reuses knockback for lunge and player shove instead of a second mover; incidental breakable hits sell room chaos without teaching enemies to farm props.
- **Alternatives**: Keep passive 0.75s contact tick only — no swing fantasy; editor-authored `CollisionShape2D` sub-resource per scene — shared-shape footgun; bespoke lunge velocity on the enemy script — duplicates knockback + nav yield; AnimationPlayer method tracks like the player arc — heavier for simple enemy spritesheets; damage still via `DamageComponent` poll on a always-on body hitbox — couples contact and swing; separate slam component for ogre — rejected, same phase machine with zero lunge distance; AI path/attack breakables on purpose — deferred; player-only swing filter — previous, props ignored swings.
- **Status**: decided (in-codebase) for goblin (lunge) and ogre (planted AOE + player knockback + earth-spike shockwave)

### Ogre earth-spike line shockwave
- **Decision**: On ogre slam strike (`EnemyAttackComponent.struck`), spawn eight reusable `LineShockwave` instances on the level (not under the ogre) along N, S, E, W, NW, NE, SW, SE. Each line staggers `EarthSpike` scenes outward into `%WorldYSort` so they y-sort with entities/props; each spike plays a one-shot sheet anim and owns a short-lived Area2D hitbox (no `DamageComponent` — same attacker-poll pattern as enemy melee). Body slam hitbox stays; spikes add reach. Damage/knockback copy the attack component exports. No wall stop, no HOLD ground telegraph in v1.
- **Why**: Readable traveling ground threat that matches the spike art; spike + line scenes stay reusable; parenting to the level keeps the wave alive if the ogre dies mid-attack; skipping the orb damage-component path avoids scaffolding a full entity for a disposable FX.
- **Alternatives**: Spikes-only (drop body box) — weaker close-range feedback; cosmetic spikes with body damage only — rays look dangerous but aren't; bake all spikes into one mega-scene — harder to reuse/tune; `DamageComponent` + attacker `HitboxComponent` on each spike — heavy for short-lived FX and fights the melee damage path; six rays without E/W — previous, left dodge corridors on the cardinals; stop at world collision — deferred.
- **Status**: decided (in-codebase)

### Desert WorldYSort for entities and ground FX
- **Decision**: Gameplay sprites that should depth-sort (player, enemies, **orbs**, props, earth spikes, cliff pieces) live under `%WorldYSort` (`y_sort_enabled`). Opening / bought / transformed orbs are parented there (same as lava globes), not the level root. Shockwave line controllers stay on the level root; spikes are parented into `%WorldYSort` (never the scene root — that draws above the whole group). Earth spikes bias their y-sort point 24 px north of the visual base so center-origin entities at the same ground line draw in front. `LevelObject` props (cactus, rocks, skull) apply a 16 px north bias (see Level objects y-sort bias). Cliff folders (`Cliffs` / `CliffSection*`) also have `y_sort_enabled` so nested pieces flatten into that sort; each `Cliff` then applies its own 16 px north bias (see cliff entry). Navmesh `navigation_source` stays on tiles/walls and on static prop scenes (not on WorldYSort) so the player/enemies are not baked as obstacles.
- **Why**: Godot y-sort only compares siblings; folder wrappers at (0,0) made spikes unable to sort against entities. A shared sort root fixes spike vs player/prop overlap. Orbs added as later children of the level always drew above WorldYSort regardless of Y.
- **Alternatives**: Per-node `z_index = int(y)` — works without a shared parent but needs every entity updated; y-sort only on the line controller — sorts spikes among themselves only; keep Objects/Players/Enemies wrappers under a sort root — wrappers share Y=0 so sorting does nothing; parent orbs to the level root — previous, always in front of cliffs.
- **Status**: decided (in-codebase)
...
- **Tether feel**: mid-combat steer is per **player character** (wizard vault / dwarf bat). Orbit capture remains parked (`capture_enabled`); entity punch-through is the default (desert export can enable bounce). Vault early-fire is **dash-again** (spends dash); timeout refunds dash CD; dwarf charge threshold **0.5 s**.
- **Attack cooldown / charges**: Attack bat mode **0.35 s** CD (no dash refund); hold ≥ 0.5 s for 360° bat; dash-again early vault fire starts dash CD; timeout auto-fire refunds dash CD; tether post-release cooldown 0.25s; melee parked.
...
- **20 specialist orbs / Attunement recipes**: `orbs` / `attunements` sheets hold element combos; playable scenes so far: Ghost / Contagion / Conduit / Vulcano / Runic. Missing combos show `???` and cannot Transform.
- **Camera / view perspective**: wizard uses SW/NW diagonal sprites; dwarf is SW-only (NW falls back to SW + flip); prototype uses a fixed centered `Camera2D` on the 640×360 arena. Confirm long-term camera for larger stages.
- **Arc deflect / melee rollback**: flip `deflect_orb_enabled` / `melee_enabled` if the Attack bat needs the old swing again.
...
- **Decision**: Reusable `@tool` scene `ui/input_icons/input_prompt.tscn` (`InputPrompt`) shows a looping 4-frame press anim from a folder PNG (`input_id` inspector enum) plus an adjacent `prompt_text` label (`pixel_medium` size 8). Sheets are fixed **64×16** (4×16×16). `configure_action(action, text, keyboard_mouse)` picks kb vs pad sheets from `ACTION_ICONS`. Summoning circle `%PromptStack` shows Buy / Activate / Cancel / Transform-or-Bake when available; icons follow the **nearest** player in `%InfoProximityArea` via `Controls.input_scheme` (last key/mouse vs that player's joypad device). No dual-icon stacks; no floating pickup/dash prompts yet.
- **Why**: Same AttributeBox-style folder picker for new art; contextual circle prompts teach binds without pausing; last-input scheme matches how P1 actually plays (kb or pad 0).
- **Alternatives**: Auto dual kb+pad icons always — clutter at 640×360; hardcode textures in circle scripts — duplicates layout; scheme from `uses_mouse()` only — never flips when P1 uses a pad; per-player prompt stacks in co-op — deferred.
- **Status**: decided (in-codebase)

### Controls: last-input InputScheme for prompts
- **Decision**: `Controls` tracks `input_scheme` (`KEYBOARD_MOUSE` / `GAMEPAD`). P1 defaults keyboard/mouse; P2+ defaults gamepad. `_input` sets keyboard/mouse on key or mouse button (P1 only) and gamepad on joypad button / stick past deadzone for that player's device (`player_index - 1`). `prefers_keyboard_mouse()` feeds InputPrompt; `uses_mouse()` stays `player_index == 1` for existing GUI gates.
- **Why**: P1 can use both keyboard and pad 0; prompts must follow the device they are actually using without breaking mouse-only GUI assumptions for P2.
- **Alternatives**: Global last-input for all players — wrong in co-op; poll action strength each frame — noisier than event-driven scheme flips.
- **Status**: decided (in-codebase)
...
- **Decision**: `HealthComponent.max_health` defaults to `1.0`, while concrete HP values are authored per scene/entity. Current examples: player `30`, goblin `25`, ogre `60`, cactus `5`, `big_rock` `60`, `animal_skull` `20`, cliff piece `80`. Orb damage to **entities** comes from the orb `DamageComponent.damage` (currently `10`) via victim `HitboxComponent` overlap polls; orb damage to **breakables** still uses `_try_apply_orb_damage` on flying `body_entered` / tether world-probe contact. On any `take_damage`, the entity sprite modulates to red (`damage_flash_color`) then tweens back (non-fatal) or stays red into the destruction FX (fatal). Sprite comes from an optional `HealthComponent.sprite` export, else `DestroyComponent.sprite`. Non-fatal hits can start gameplay i-frames (see below).
...
- **Decision**: Every successful `HealthComponent.take_damage` spawns a detached world-space number (`DamageLabelEffect` → `damage_label.tscn`) at the owner, parented to `current_scene` so killing blows still show after `queue_free`. Uses `pixel_medium.fnt` at size 8; rises ~20 px over 1 s and fades in the last 0.3 s (`ignore_time_scale`). `DamageKind` selects color: **STANDARD** white (default), **BURN** orange (Burn DoT ticks), **SHADOW** black (Ghost possession DPS + Ghost `DamageComponent`), **CRIT** gold. **POISON** green remains in the enum unused. Black numbers use a light outline for readability on dark possessed sprites; others use a dark outline. Callers pass the kind (`StatusComponent` Burn → BURN, `GhostOrb` possession → SHADOW, `DamageComponent.damage_kind` on hitbox/orb breakable paths).
- **Why**: Instant readable confirmation of chip amounts and elemental source without cluttering permanent HUD; shared HealthComponent path covers player, enemies, breakables for free.
- **Alternatives**: Always-white numbers — loses burn/shadow readability; color by attacker type at the label only — duplicates kind logic outside HP; parent labels to the entity — vanish on death before the float finishes; screen-space HUD popups — harder to attribute in a crowded arena.
- **Status**: decided (in-codebase)
...
- **Decision**: `BlankOrb` is a `RigidBody2D` with a circular body shape (**world + orb** mask while flying; flying can also mask player + enemy when `bounce_off_entities` is true), locked rotation, gravity 0, friction 0, **bounce 0** material, continuous CCD, `can_sleep` off, and constant-speed flight. All reflection is owned by `_integrate_forces`: inbound contact normals are summed, then `aim_direction` reflects **once** off that combined normal (only when moving into the surface), then `linear_velocity = aim_direction * speed`. `_physics_process` does **not** re-derive direction from solver velocity. World bounce SFX and breakable damage use `body_entered` (skips player / enemies / other orbs for HP). Player/enemy damage uses the orb's `HitboxComponent` Area2D via victim hitbox polls (punch-through or bounce). Tether probes stay **world-only**. Opening co-spawned orbs get a brief mutual collision exception (~0.2 s) so the volley fans out. Post-launch grace adds a collision exception vs the batter so entity bounce does not immediately rebound off the redirecting player.
- **Why**: Solver bounce + script bounce fought each other and caused sticky re-entry (double damage / double SFX). Real contact normals beat center-to-center approximations on capsules and rectangles. World solids still reshape paths; orb–orb bounce makes multi-orb rooms chaotically interactive without mutual HP; the desert export restores entity bounce for A/B.
- **Alternatives**: PhysicsMaterial bounce 1 + velocity read-back — previous; double-reflect artifacts; world + player + enemy mask with entity bounce always — previous punch-stop feel; world-only mask without hitbox entity damage — orb phases harmlessly through targets; punch through other orbs (previous) — weaker multi-orb pinball; orb–orb damage — rejected, orbs are not HP entities.
- **Status**: decided (in-codebase)
...
- **Decision**: After every launch (opening tether release, deflect, mid-combat tether release / forced break, **or summoning-circle ritual release**), the orb ignores the releasing player for **1.0 s** (`player_grace_seconds`). While the countdown runs, the orb shows a ghostly tint and a shrinking halo ring that tracks remaining grace. While tethered, `instigator` stays pinned to the tethering player for the whole orbit (countdown cleared mid-tether so the visual does not run during orbit). Circle cancel passes the cancelling player into `release_from_circle` → `begin_flight(dir, by)`.
- **Why**: Deflecting / tethering / standing in the circle to release requires proximity; without grace the player dies on the same frame they bat or release the orb. 1 s gives a readable dodge window after a sling; the orb-side visual makes “this orb won’t hurt you yet” obvious without implying full player i-frames.
- **Alternatives**: 0.3 s grace — too short once orbit radius and release tangents put the player in the path; teleport orb away on deflect — less readable; disable player hit forever until leave range — easier to cheese; player-side blink — reads as full invulnerability to enemies too; circle release with `begin_flight(dir, null)` — previous, instantly damaged players standing on the circle.
- **Status**: decided (in-codebase)

### KnockbackComponent for shove without damage
- **Decision**: `KnockbackComponent` applies a decaying shove to its owner (`CharacterBody2D` via velocity + `move_and_slide`, `RigidBody2D` via impulse, plain `Node2D` via position). Enemies yield chase AI while knockback is active. The **player** also owns one so enemy slam `hit_knockback_distance` can shove them (Walk/Idle yield; dash `cancel()`s). Orb weight knockback still targets **enemies only**. `cancel()` clears an in-progress shove.
- **Why**: Attack needs a non-damage response for enemies; ogre slam needs the same shove on the player without a second mover; keeps shove logic out of MovementComponent and entity scripts.
- **Alternatives**: Bake knockback into MovementComponent — couples walk and shove; one-off velocity in grunt script — not reusable for props; apply player shove by writing velocity in the attack component — duplicates KnockbackComponent.
- **Status**: decided (in-codebase)

### Dash: fixed distance with i-frames and phase-through
- **Decision**: Player dashes 50 px toward **current facing** (8-way via `DirectionalSpriteComponent.facing_vector()`) at 400 px/s via `DashComponent` + `Dash` state. Idle dash uses last facing; dash does not read mouse/aim. While dashing: hitbox monitoring off (immune to orb and enemies), body `collision_layer` cleared and `collision_mask` set to the `wall` layer so the player phases through props/enemies but is stopped by outer arena walls, no movement input, no attacking. Dash duration is consumed by intended distance (not actual travel) so a wall block cannot stall the dash. End-of-dash placement is resolved before collision is restored (see "Dash never ends inside world geometry"). Cooldown is **1.5 s** on the player scene (component default 4 s), shown as a radial ring above the head drawn by `DashComponent` itself. Starts only from Idle/Walk (does not cancel an attack swing), or from vault hold via dash-again. Left click / **R2** is `dash`; Attack is **F / R1** (orb bat, not melee). Outer `Walls` bodies sit on both `world` and `wall` so walking, orb bounce, and nav bake are unchanged. While `dash_vault_enabled`, a successful vault **timeout** calls `end_and_clear_cooldown()` so the player can dash out immediately; **dash-again** during the hold fires the orb then starts a real dash (cooldown begins). Attack bat does **not** refund dash. **Cannot dash while carrying a glyph.**
- **Why**: Readable dodge through the orb, chasers, and clutter; fixed distance is easy to learn and tune; facing-direction dash matches movement/strafe intent and frees aim for the attack arc; phase-through keeps the dash reliable in a prop-filled arena; leaving the arena by dashing through the border slabs was a hole; a cooldown ring makes the recharge obvious. Cooldown UI lives on `DashComponent` because dash is player-only and 1:1 with that cooldown state. Clearing cooldown on vault timeout enables an immediate dodge after the bat; dash-again spends that refund for an early exit. Blocking dash while carrying makes delivering glyphs to the circle a committed walk.
- **Alternatives**: Aim-direction dash (mouse / right stick) — coupled dodge to attack aim and fought strafe play; reuse `KnockbackComponent`'s decaying shove — wrong curve (fade-out vs constant speed) and no i-frame API; velocity-based dodge that keeps movement control — less committed and harder to read; interrupt attack with dash — too many cancel options for prototype; keep world collision during dash — props truncate the dash unpredictably; clear all collision during dash — previous; let the player leave the arena; separate `DashCooldownIndicator` component — extra scene wiring for a UI that nothing else reuses (health bar stays separate because it is shared across entities); gamepad B for dash — previous face-button bind, superseded by R2.
- **Status**: decided (in-codebase)

### Dash never ends inside world geometry
- **Decision**: Before `_end_phase()` restores the player's `world` collision mask, `DashComponent` resolves the end position via shared `CollisionSeparation`: if the capsule overlaps world/wall, scan **forward** up to `unstick_forward_distance` (24 px) for a clear point (rejecting candidates whose sweep crosses the `wall` layer so border slabs cannot be tunnelled), else scan **backward** up to `dash_distance`, else last-resort rest-normal push. A player-side stuck watchdog also runs `_physics_process` shape queries and calls `CollisionSeparation.separate` after 6 consecutive overlapping frames while not assembling/dashing (covers knockback into props, co-op spawn offset, navmesh rebake). Enemy spawn depenetration reuses the same helper.
- **Why**: Phase-through dash always consumes intended distance, so restoring collision inside a rock/cactus/cliff leaves the capsule deeply embedded; Godot `move_and_slide` recovery only resolves sub-pixel overlap and the player stays stuck. Forward-then-back preserves the "I dashed through it" read for typical props while cliffs (wider than 24 px) fall back to the last clear point on the path.
- **Alternatives**: Dash mask includes `world` so props truncate the dash — reverses the recorded phase-through decision; back-only snap — never gains ground through a prop; rely on `move_and_slide` recovery alone — insufficient for deep embedding; dash-only fix without watchdog — leaves non-dash stuck cases unresolved.
- **Status**: decided (in-codebase)
...
### Level objects: y-sort bias matches center-origin entities
- **Decision**: `LevelObject` applies `SORT_BIAS_Y` **16 px** north at runtime after picking a variant. Sprite offset and body collision shift south by the same amount so art/physics stay put; `Breakable` also shifts `HealthBarComponent` and `HitboxComponent`. Editor placement stays at the visual base. `Cliff` keeps its own bias path (does not call `LevelObject._ready()`).
- **Why**: Props author origin at the **bottom** of the art; player/enemies sort from **sprite center**. A player standing just south of a cactus has feet on the trunk but a smaller Y than the cactus origin, so y-sort drew the cactus on top.
- **Alternatives**: Bias only cactus — rocks/skulls share the same origin mismatch; shrink collision so the player cannot overlap the sprite from the south — does not fix y-sort at contact; y-sort the sprite child instead of the body — splits depth from the collider; move player origin to feet — larger entity change; 8 px minimum (player feet-box north edge is 5 px south of origin) — tight at contact vs the existing 16 px cliff/entity convention.
- **Status**: decided (in-codebase)

### Cliff pieces: inspector kind + runtime adjacency frames
- **Decision**: Destructible cliff segments are a dedicated `Cliff` breakable (`cliff.tscn` / `cliff.gd`). Authors pick `kind` among seven sheets — `corner_SW` / `corner_SE` / `wall_1` / `wall_2` / `corner_NW` / `corner_NE` / `vertical_wall` — and may preview `frame_index` in the inspector; an `@tool` apply sets the vertical-strip `Sprite2D` (`hframes` 1 / `vframes` 4 for corners and horizontals, **2** for `vertical_wall` at 32×32 cells) plus body/hitbox rects. Does **not** call `LevelObject._ready()` / random `LevelObjectVariant`. Default HP **80**. Optional collision size/offset overrides. **At runtime**, adjacency kinds overwrite `frame_index` from 32 px neighbors (group `cliffs`; left / right / above / below, 4 px slop). `vertical_wall` skips adjacency framing and instead picks `randi() % 2` once on ready (still counts as a neighbor for others). `Cliff.refresh_all_frames` runs deferred on ready and when any cliff is destroyed (dying cliff leaves `cliffs` immediately and is skipped if queued for deletion). Frame rules: SW/NW corners map both-connected → 0, either relevant side → 1, neither → 3; SE/NE distinguish the two single-neighbor cases as 1 vs 2; horizontals use left+right (0–3). Desert `%WorldYSort/Cliffs` and each `CliffSection` have `y_sort_enabled` so pieces flatten into WorldYSort; each cliff then shifts its origin **16 px north** at runtime (`SORT_BIAS_Y`) and adds that back onto sprite/collision/hitbox/health-bar so art and physics stay put. Editor placement stays at the visual base. Cliff roots do **not** enable y-sort (would split sprite from body).
- **Why**: Kind is placement authorship; frame must match living neighbors so destroying a segment reveals the correct cut faces without hand-wiring each instance. Vertical fill pieces only need cosmetic variety, not expose-side logic. Editor frame remains for layout preview before play. Nested folder y-sort is required so cliffs interleave with entities; the 16 px bias matches the authored 16 px collision lip so center-origin player/enemies standing just south of a face draw in front instead of behind the wall.
- **Alternatives**: Separate `vertical_wall_W` / `vertical_wall_E` with above/below adjacency — superseded by one randomized `vertical_wall` sheet; TileMap terrain peering — cannot carry per-piece HP / destroy VFX; random `LevelObjectVariant` — wrong for authored cliff shapes; flatten every cliff onto WorldYSort — works but loses section folders; y-sort the Cliff root — sorts the sprite separately from the body; 24 px spike-style bias — tighter margin on the plateau north of the lip.
- **Status**: decided (in-codebase)

### Breakables bounce then destroy via orb body contact
- **Decision**: Breakables (cactus) live on the `world` physics layer like rocks/walls. The orb damages them via flying `RigidBody2D.body_entered` → `_try_apply_orb_damage` (and the tether world-probe path), not the Area2D hitbox poll, so the bounce impulse is solved before `queue_free()`. Each breakable also owns a `HitboxComponent` on the world layer with **collision_mask 0** so `EnemyAttackComponent` swings can find it without the breakable polling orb `DamageComponent` hitboxes (which would double-hit with body contact).
- **Why**: Props should reshape orb paths; using a leading hitbox-only destroy would free the body before bounce. No fifth physics layer needed. Entities use hitbox poll instead because they are punch-through. Enemy incidental smash reuses the same HealthComponent.
- **Alternatives**: Punch-through breakables too — less spatial play; dedicated `breakable` layer + hitbox area — destroys before bounce; multi-hit health — deferred; breakable hitbox that also masks orbs — double ticks with body contact.
- **Status**: decided (in-codebase)
...
### Player facing: 8-way logical / SW+NW art with east flip
- **Decision**: Logical facing is **8-way** (N, S, E, W, NE, NW, SE, SW) via octant snap in `DirectionalSpriteComponent.face()`. Authored body clips are **SW and NW only** for `idle` / `moving` / `attacking`; **SE mirrors SW** and **NE mirrors NW** via `AnimatedSprite2D.flip_h`. Playback names are `<action>_sw` / `<action>_nw`; cardinals keep the other-axis bias of the previous visual (e.g. N → `ne` or `nw`) then resolve to west clip ± flip. Facing comes from movement while walking and snaps to aim on attack/redirect/glyph throw; dash uses `facing_vector()` and does not retarget facing; idle keeps the last facing. `MovementComponent` does not own the player sprite (no walk flip). `DashComponent` does not set `flip_h`. Successful orb redirect and glyph throw call `Player.play_attack_visual()` (face aim + restart `attacking`); Idle/Walk skip face/play while that clip is still playing, then resume.
- **Why**: Dash and movement need true cardinals; only west diagonals are authored; mirroring west→east matches enemy-style flip economy without inventing SE/NE sheets; action names match the SpriteFrames resource (`moving` / `attacking`, not `walk` / `attack`).
- **Alternatives**: Four fully authored diagonal sheets — art not ready; unique `*_se` / `*_ne` clips without flip — previous decision, superseded once east art was deferred; 4-way logical facing only — no straight N/S/E/W dash; always face aim — would spin the sprite while strafing.
- **Status**: decided (in-codebase)
...
- **Status**: decided (in-codebase); statuses: Burn 1 dmg/stack/s; Chill 5% move+attack slow/stack (max 90%); Blight −5% dealt / +5% orb taken per stack (max 90%; orb impact/splash/bowling only); Shock stuns at 10 stacks for 2 s after 50 burst damage; Disease permanent flag, on death spreads floor(blight/2) within 100 px (no chaining). All stacks persist until death.
