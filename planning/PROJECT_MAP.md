...
- `project/areas/level/level.gd` — director: bakes navigation, starts `EnemySpawner`, awaits parallel `begin_level()`; applies `RunStartConfig` to the circle then launches configured orbs (default **Ghost + Contagion + Conduit**) from `%SummoningCircle` into `%WorldYSort` with a brief mutual collision exception (~0.2 s) so co-spawned orbs fan out; on kills rolls **glyph** spawn via orb `glyph_drop` or `glyph_drop_chance` export; weighted glyph rarity **70 / 25 / 5**; `SummoningCircle.ritual_started` → live circle UI (no pause); `new_blank_orb_requested` / `transform_requested` from circle (Transform instantiates the specialist then `commit_transformed_orb` launches it); `pause` → `%RitualMenu.open_inspect`; co-op P2 hot-join; win/lose when all players dead; orb tether/time-slow hooks
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `InputScheme` last-input tracking (`KEYBOARD_MOUSE` / `GAMEPAD`; P1 defaults kb/mouse, P2+ gamepad); `prefers_keyboard_mouse()` for prompts; `uses_mouse()` (P1 only, GUI gates); `get_move_vector()`, `get_aim_vector(origin)`, `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()` / activate / upgrade / ritual_cancel
- `project/entities/player/states/idle.gd` — stops movement (yields while knockback active); plays `idle` when not mid-`attacking`; transitions to walk, dash; movement locked while tethering / channeling / **vaulting**; dash blocked while vaulting (vault dash-again handled in `OrbTetherComponent._update_vault`)
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()` (yields while knockback active); plays `moving` and `face(dir)` when not mid-`attacking`; transitions to idle, dash; forces idle while tethering / channeling / **vaulting**; dash blocked while vaulting
- `project/entities/player/states/attack.gd` — snapshots aim, `play("attacking")` + starts `AttackComponent`; returns to idle when swing ends (kept for `melee_enabled`; no input currently transitions here)
...
- Player input always goes through the `Controls` node's `PlayerAction` children; never hardcode action strings in state or component scripts. `apply_player_index(n)` appends `""` / `"_2"` / `"_3"` / `"_4"` suffix at `_ready` time. Actions: move / aim / **pickup** / dash / activate / upgrade / ritual_cancel.
