...
### Cliff pieces: inspector sheet + frame (not random variants)
- **Decision**: Destructible cliff segments are a dedicated `Cliff` breakable (`cliff.tscn` / `cliff.gd`). Authors pick `kind` (`corner_SW` / `corner_SE` / `wall_1` / `wall_2`) and `frame_index` in the inspector; an `@tool` apply sets the vertical-strip `Sprite2D` (`hframes` 1 / `vframes` per sheet) plus body/hitbox rects. Does **not** call `LevelObject._ready()` / random `LevelObjectVariant`. Default HP **80**. Optional collision size/offset overrides. Neighbor expose-texture swap and multi-piece formations are deferred.
- **Why**: Cliff art is authored per placement (which sheet + which frame), not a random cosmetic roll like cactus variants; `@tool` preview keeps layout readable while placing.
- **Alternatives**: Random `LevelObjectVariant` array — wrong for authored cliff shapes; TileMap terrain peering — cannot carry per-piece HP / destroy VFX; one mega-scene with every frame as a child — heavier to instance and tune.
- **Status**: decided (in-codebase) for single authored piece; neighbor texture swap planned
