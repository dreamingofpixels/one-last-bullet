...
│   ├── attack_component.gd / .tscn   Player arc swing; hitbox/knockback behind melee_enabled; `play_swing_visual` for Attack bat arc art; orb deflect gated by flag; Attack bat consumes cooldown
...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (charge → 180° or 360°); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / `orb_redirect`); **pickup** glyph grab/throw + ritual socket
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`orb_redirect_mode`** (`ATTACK_BAT` / `DASH_VAULT`, mutual exclusive; set by player `character`); **`capture_enabled`** (false on player) gates tap capture + remote channel; **attack** charge hold then `try_attack_bat(front_cone)` — short release = 180° cone + `attacking`, hold ≥ 0.5 s = 360° + `attack_around`; plays `attack_bat_sound` (axe whoosh) once per bat and `attack_bat_redirect_sound` at each batted orb; **pickup** press order: release owned tether → throw if carrying → **ritual socket/unsocket** if circle has a captured orb → **glyph** pickup → orb capture (if capture on); circle **activate** is on **`activate`** (Triangle / Space) including Transform/bake; buy on **`upgrade`** (Square / E); …
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `InputScheme` last-input tracking (`KEYBOARD_MOUSE` / `GAMEPAD`; P1 defaults kb/mouse, P2+ gamepad); `prefers_keyboard_mouse()` for prompts; `uses_mouse()` (P1 only, GUI gates); `get_move_vector()`, `get_aim_vector(origin)`, `get_explicit_aim_vector(origin)` / `is_explicitly_aiming()` (bat chevron + launch), `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()` / `is_attack_just_pressed()` / `is_attack_pressed()` / `is_attack_just_released()` / activate / upgrade / ritual_cancel
- `project/entities/player/states/idle.gd` — stops movement (yields while knockback active); plays `idle` when not `is_attack_busy()`; transitions to walk, dash; movement locked while tethering / channeling / **vaulting**; dash blocked while vaulting (vault dash-again handled in `OrbTetherComponent._update_vault`); Attack bat polled in tether `_process`
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()` (yields while knockback active); plays `moving` and `face(dir)` when not `is_attack_busy()`; transitions to idle, dash; forces idle while tethering / channeling / **vaulting**; dash blocked while vaulting; Attack bat polled in tether `_process`
...
- `project/entities/orbs/aim_arrow.gd` — three pulsing chevrons outside the orb along player aim (`set_redirect_preview`); used by vault hold and Attack-bat preview (closest cone orb while explicitly aiming)
