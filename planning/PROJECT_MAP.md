...
│   ├── collision_separation.gd      Static shape-query helpers: is_clear / resolve_along / separate (dash unstick, player watchdog, enemy spawn depenetration)
    ...
    │   │   ├── controls.gd         Per-player input abstraction (move/aim/pickup/dash)
    │   │   └── states/
    │   │       ├── idle.gd
    │   │       ├── walk.gd
    │   │       ├── attack.gd
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`orb_redirect_mode`** (`ATTACK_BAT` / `DASH_VAULT`, mutual exclusive; level can override); **`capture_enabled`** (false on player) gates tap capture + remote channel; **attack** (`try_attack_bat`) bats flying orbs in a 180° front half-circle along explicit aim when in Attack Bat mode; **pickup** press order: release owned tether → throw if carrying → **ritual socket/unsocket** if circle has a captured orb → **glyph** pickup → orb capture (if capture on); circle **activate** is on **`activate`** (Triangle / Space) including Transform/bake; buy on **`upgrade`** (Square / E); …
...
### Entities
- `project/entities/player/player.tscn` + `player.gd` — … carry API for **glyphs** (`pick_up_item` / `try_throw_item` / `is_carrying_item`); **`play_attack_visual(aim)`** faces aim and restarts body `attacking` (used by glyph throw / future melee); **KnockbackComponent** for enemy slam shove; stuck watchdog (`CollisionSeparation`) unsticks after 6 consecutive world-overlap frames while not assembling/dashing; …
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `InputScheme` last-input tracking (`KEYBOARD_MOUSE` / `GAMEPAD`; P1 defaults kb/mouse, P2+ gamepad); `prefers_keyboard_mouse()` for prompts; `uses_mouse()` (P1 only, GUI gates); `get_move_vector()`, `get_aim_vector(origin)`, `get_explicit_aim_vector(origin)` / `is_explicitly_aiming()` (bat chevron + launch), `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()` / `is_attack_just_pressed()` / activate / upgrade / ritual_cancel
- `project/entities/player/states/idle.gd` — stops movement (yields while knockback active); plays `idle` when not mid-`attacking`; transitions to walk, dash; movement locked while tethering / channeling / **vaulting**; dash blocked while vaulting (vault dash-again handled in `OrbTetherComponent._update_vault`); Attack bat polled in tether `_process`
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()` (yields while knockback active); plays `moving` and `face(dir)` when not mid-`attacking`; transitions to idle, dash; forces idle while tethering / channeling / **vaulting**; dash blocked while vaulting; Attack bat polled in tether `_process`
...
| `attack` | **F**, gamepad **R1** (device 0, button 10) — bat flying orbs in front half-circle | gamepad **R1** device 1 |
| `dash` | left click, gamepad **R2** (device 0, axis 5) | gamepad **R2** device 1 |
