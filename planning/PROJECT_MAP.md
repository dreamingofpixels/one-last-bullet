...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack redirect; `dash_vault_enabled` gates vault vs proximity; vault uses remaining-ray early stop + 180° aim cone; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; **pickup** glyph grab/throw + circle activate
...
| `navigation_source` | Desert navmesh contributors (`LowerGround`, `Cliffs`, `Walls`, plus cactus / big_rock / animal_skull scenes) |
