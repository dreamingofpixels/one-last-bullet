...
- `project/objects/summoning_circle/summoning_circle.tscn` + `.gd` — floor circle with `%DepositArea` (radius 22), `%InfoProximityArea` (radius 64, player-only, OrbInfo visibility), `%ManaPoolLabel`, `%ArcaneParticles`, `%OrbInfo`, `%GlyphSlots` / `%GlyphSocket1-3` / `%GlyphIcon1-3` (`socket_interact_radius` 12 for Cross place/remove on the nearest socket), `%BuyBlankOrb`; owns `mana_pool` (no glyph inventory); live ritual per-socket Cross / Transform/bake/buy; signals `ritual_started` / `ritual_ended` / `new_blank_orb_requested` / `transform_requested`; group `summoning_circle`
...
- `project/ui/inventory/orb_inventory.tscn` + `.gd` — **`OrbInventoryBar`**: 3 orb sockets + mana label; glyph sockets hidden via `set_glyph_sockets_visible(false)` in inspect
...
| `upgrade` | **F**, gamepad **Y / Triangle** (device 0) — activate / buy / Transform/bake | gamepad **Y** device 1 |
