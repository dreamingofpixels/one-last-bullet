...
- **Tether feel**: playtest mid-combat steer is **dash vault** (`dash_vault_enabled`); proximity Attack redirect parked behind the same flag. Orbit capture remains parked (`capture_enabled`); entity punch-through is the default (desert export can enable bounce).
- **Attack cooldown / charges**: early vault fire 0.35s; timeout auto-fire no attack CD; tether post-release cooldown 0.25s; melee parked.
