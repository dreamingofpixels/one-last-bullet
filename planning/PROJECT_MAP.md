...
│   ├── attack_component.gd / .tscn   Player arc swing; `%CollisionPolygon2D` geometry hit + persistent overlay for Attack bat (white flash on connect); hitbox/knockback behind melee_enabled; `play_swing_visual` for arc art; orb deflect gated by flag; Attack bat consumes cooldown
...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (charge → swing polygon or 360° at `attack_spin_radius`; move/dash locked while charging; `attack_bat_hold_seconds` orb hitstop before deflect); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; Attack bat SFX (`axe_swing` / pitched `orb_redirect` at fly-off); **pickup** glyph grab/throw + ritual socket
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`attack_spin_radius` (32 px)** for charged 360° bat (body-disk overlap: spin radius + orb radius); **`attack_bat_hold_seconds` (0.15)** orb-only hitstop after a connecting bat (multi-orb `_bat_holds`; live tap aim / spin bounce unless stick; delayed `deflect` + pitched `orb_redirect`); **`orb_redirect_mode`** (`ATTACK_BAT` / `DASH_VAULT`, mutual exclusive; set by player `character`); **`capture_enabled`** (false on player) gates tap capture + remote channel; **attack** charge hold (**move/dash locked**) then `try_attack_bat(front_cone)` — short release = swing **polygon** + `attacking`, hold ≥ 0.5 s = 360° + `attack_around`; `get_bat_aim()` shared with overlay (stick/mouse, latches last aim); plays `attack_bat_sound` (axe whoosh) once per bat and `attack_bat_redirect_sound` at fly-off; **pickup** press order: release owned tether → throw if carrying → **ritual socket/unsocket** if circle has a captured orb → **glyph** pickup → orb capture (if capture on); circle **activate** is on **`activate`** (Triangle / Space) including Transform/bake; buy on **`upgrade`** (Square / E); …
