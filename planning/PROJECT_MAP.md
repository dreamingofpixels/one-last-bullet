...
- Player facing: 8-way logical facing; visuals use authored `<action>_sw` / `<action>_nw` (`idle` / `moving` / `attacking` / `attack_around`) with `flip_h` for SE/NE; missing NW falls back to SW; `DirectionalSpriteComponent` owns octant snap, cardinal→diagonal mapping, east flip, and charge pause-hold; **wizard** faces movement while walking; **dwarf** faces bat aim (`Player.sync_body_facing`); redirect/throw call `Player.play_attack_visual()` (dwarf spin uses `play_attack_around_visual()`).
