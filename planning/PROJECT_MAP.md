...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `uses_mouse()` (P1 only); `get_move_vector()`, `get_aim_vector(origin)`, `get_throw_aim_vector()` (right stick else move dir), `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()`
...
- `project/objects/summoning_circle/summoning_circle.tscn` + `.gd` — floor circle with `%DepositArea` (radius 22, activate / deposit / capture), `%InfoProximityArea` (radius 64, player-only; OrbInfo, BuyBlankOrb, **Triangle Transform/bake**, **Square buy**, **Circle cancel/disarm**), `%ManaPoolLabel`, `%ArcaneParticles`, `%OrbInfo` (`%OrbNameLabel` + `%EffectLabel` + stats; `%FireHint` / `%WaterHint` / `%AirHint` / `%EarthHint` on each element row — shown only at exactly two socketed glyphs via `OrbRecipes.hints_for`; at three playable glyphs OrbInfo name/effect/stats preview the specialist), `%GlyphSlots` / `%GlyphSocket1-3` / `%GlyphIcon1-3` (`socket_interact_radius` 16 for L2 place/remove; sockets ~33 px from center, outside DepositArea; GlyphSlots use `z_as_relative = false` like the floor sprite so they draw behind the player), `%BuyBlankOrb` (idle / waiting, only while a player is in InfoProximityArea); owns `mana_pool` (no glyph inventory); **`MAX_ORBS` 5**; live ritual: Triangle/`activate` arms capture or Transform/bake then **launches**, Square/`upgrade` buys while idle/waiting, Circle/`ritual_cancel` disarms waiting or releases (Transform via `commit_transformed_orb`; bake then `release_orb`; capture grace on launch); signals `ritual_started` / `ritual_ended` / `new_blank_orb_requested` / `transform_requested`; group `summoning_circle`
