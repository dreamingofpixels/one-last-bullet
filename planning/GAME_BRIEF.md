...
- Starting mana, opening orbs, and circle glyph inventory are authored on the level via **`RunStartConfig`** (`run_start_default.tres`: 0 mana, Ghost/Rot/Conduit, one Common Air `static`). Mana label in the inventory bar updates live on deposit/spend.
...
- **Opening volley**: design now starts with **one Blank Orb** from the circle; prototype launches orbs from `RunStartConfig` (default Ghost / Rot / Conduit). Reconcile after playtest.
