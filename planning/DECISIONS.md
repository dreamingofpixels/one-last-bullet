...
- **Status**: superseded — goblin now uses committed `EnemyAttackComponent` melee (see below); brute still contact-ticks

### Enemy committed melee via EnemyAttackComponent
- **Decision**: Enemies that own an attack clip use `EnemyAttackComponent` (Area2D): proximity enter → stop chase → play attack anim → **hold on an authored lunge frame** → play attack SFX → activate a **code-built rect hitbox** (editor `hitbox_size` / `hitbox_offset`, mirrored by facing) while `KnockbackComponent.push_distance` lunges toward the snapshotted player position → finish anim → cooldown → resume chase. Passivebox damage is applied directly to the player `HitboxComponent` / `HealthComponent` (once per swing); player dash i-frames still skip via `monitoring == false`. Goblin drops its `DamageComponent` so passive contact no longer ticks. Per-enemy `SoundEvent` on the component (`goblin_knife_attack.tres`). Chill slows via `speed_multiplier`; Shock stun calls `cancel()`. Spawner `set_active(false)` while assembling.
- **Why**: Readable telegraphed threat that matches the attack art; one reusable component for future enemies; authored box size without sharing a mutated `RectangleShape2D` sub-resource across instances; reuses knockback for the lunge instead of a second mover.
...
- **Status**: decided (in-codebase) for goblin; brute still contact-only until it gets a clip + component
