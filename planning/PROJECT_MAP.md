...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack bat (charge → 180° or 360°); `orb_redirect_mode` from player character; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; **pickup** glyph grab/throw + ritual socket
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`orb_redirect_mode`** (`ATTACK_BAT` / `DASH_VAULT`, mutual exclusive; set by player `character`); **`capture_enabled`** (false on player) gates tap capture + remote channel; **attack** charge hold then `try_attack_bat(front_cone)` — short release = 180° cone + `attacking`, hold ≥ 0.5 s = 360° + `attack_around`; **pickup** press order: release owned tether → throw if carrying → **ritual socket/unsocket** if circle has a captured orb → **glyph** pickup → orb capture (if capture on); circle **activate** is on **`activate`** (Triangle / Space) including Transform/bake; buy on **`upgrade`** (Square / E); …
