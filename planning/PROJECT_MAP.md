...
│   ├── enemy_attack_component.gd / .tscn  Enemy committed melee: proximity windup → hold frame → rect hitbox; damages player + breakables; optional self-lunge (`lunge_distance`) and victim shove (`hit_knockback_distance`); Chill speed_multiplier + stun cancel
...
- `project/components/enemy_attack_component.gd` — enemy `Area2D` committed melee; mask `player|world`; `_poll_hits` damages `player` and `breakables` once per swing; editor `hitbox_size` / `hitbox_offset` build a per-instance `RectangleShape2D`; proximity → windup → hold → strike; optional self-lunge (`lunge_distance`) and victim radial shove (`hit_knockback_distance`); `set_active` / `cancel` / `speed_multiplier`; editor `@tool` gizmo outline
...
- Wired events: entity destroyed/damaged, dash, orb bounce (`impact_soft`), begin/release tether, item pickup, mana crystal deposit; goblin knife attack (`goblin_knife_attack.tres`); attack swing export exists but unassigned
