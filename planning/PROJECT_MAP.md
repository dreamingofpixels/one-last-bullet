...
│   ├── orb_tether_component.gd / .tscn  Focus + dash vault / Attack redirect; `dash_vault_enabled` gates vault vs proximity; vault uses remaining-ray early stop + 360° aim; dash-again during hold fires early then starts a real dash (CD); `vault_recatch_cooldown` blocks re-grabbing the same orb; capture gated by capture_enabled; **pickup** glyph grab/throw + circle activate
...
- `project/components/orb_tether_component.gd` — focus radius (48 px on player scene); **`dash_vault_enabled`** (true on player) replaces proximity Attack redirect with dash-into-orb vault (`prepare_vault_dash` commits + clamps remaining to far-side landing; dash walks there; `try_finish_vault_dash` starts 0.5 s aim hold with **360°** aim; **dash-again** early-fire + real dash / timeout auto-fire; dash-away faces **move stick / WASD** when held else last facing; **`vault_recatch_cooldown` 0.35 s** skips that same orb on the follow-up dash); **`capture_enabled`** (false on player) gates tap capture + remote channel; **pickup** press order: release owned tether → throw if carrying → **summoning circle `try_activate()`** if in `DepositArea` → **glyph** pickup → orb capture (if capture on); …
...
| `attack` | left click, gamepad X (device 0) — **parked** | gamepad X device 1 — parked |
| `pickup` | right click, gamepad Y (device 0) | gamepad Y device 1 |
