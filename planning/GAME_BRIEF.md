...
- **During the level**, activate the **summoning circle** (first activation free, then **+5 mana** each time). The next orb that enters opens the **ritual menu**:
  1. **Socket glyphs** by dragging from the bottom glyph inventory into any of the focused orb's **3 independent slots** (mouse hold-drag lands on the exact slot; occupied slot **swaps**; or controller d-pad / left-stick **flick** to move focus — one step per flick — / dash to grab → up to snap to first empty slot → left/right across recycle and slots → dash to drop; tether cancels). Socketed glyphs can be **picked back up** (mouse-drag off the slot, or controller focus + dash — holds and highlights the glyph so it can be moved again; down sends it to inventory). Three on a Blank Orb → Transform into a specialist when the element combo is playable. Three on a non-Blank with the right combo → **Attunement** (Transform gated the same way once attunements have scenes). After two glyphs, show four possible result hints (`???` vs name).
