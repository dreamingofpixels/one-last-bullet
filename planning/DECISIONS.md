...
### Attack bat (180° move-facing cone; A/B vs vault)
- **Decision**: Mid-combat Attack is **R1 / F** (`attack` / `attack_2`) when `orb_redirect_mode == ATTACK_BAT`. Pressing Attack bats **every** flying orb in a **180° half-circle** in front of the player within `focus_radius` (**48 px**). Forward = live move / left stick; if standing still, `DirectionalSpriteComponent.facing_vector()`. Launch direction is `Controls.get_explicit_aim_vector()` (right stick past deadzone; else mouse only while `input_scheme == KEYBOARD_MOUSE`); if no explicit aim, use cone forward. Each hit calls `BlankOrb.deflect()` (**+10%** speed/damage). Shares **0.35 s** attack cooldown via `consume_cooldown()`. Does **not** reset dash cooldown. Plays `AttackComponent.play_swing_visual()` (arc sprite + AnimationPlayer) plus body `play_attack_visual`; hitbox / enemy knockback stay off (`melee_enabled = false`). Chevrons show on the **closest** cone-eligible orb **only while explicitly aiming** (`is_explicitly_aiming()`). Blocked while assembling, dashing, vault-aiming, tethering, channeling, or carrying a glyph. Mutually exclusive with dash vault via `OrbTetherComponent.orb_redirect_mode` (level export applies to all players). Implemented in `OrbTetherComponent.try_attack_bat()`.
- **Why**: Gives a walk-up bat that reads as a front half-swing without restoring melee; cone from movement matches “hit what’s in front of you”; explicit-aim-only chevron avoids a constant arrow when the pad stick is centered; A/B enum keeps one steer fantasy clear while deciding; arc art sells the bat without enemy knockback.
- **Alternatives**: Closest-only redirect — previous proximity bat; always-on chevron when in range — previous, noisy on pad; refund dash on bat — previous proximity redirect; run bat and vault together — previous briefly, rejected for muddy A/B; enable full `melee_enabled` for sprite — would restore knockback; body clip only — previous, arc sprite stayed hidden; bind Attack to Cross — left unused on purpose; use LMB — already dash.
- **Status**: decided (in-codebase)

### Orb redirect mode playtest toggle
- **Decision**: `OrbTetherComponent.OrbRedirectMode` enum (`ATTACK_BAT` / `DASH_VAULT`) replaces the old `dash_vault_enabled` bool as the mid-combat steer switch. `dash_vault_enabled` / `attack_bat_enabled` are read-only getters from the enum. Desert / `level.gd` export **`orb_redirect_mode`** applies to every player in `_ready` and on P2 join (same pattern as `bounce_orbs_off_entities`). Default **Dash Vault**.
- **Why**: One inspector knob for A/B without digging into each player’s tether component; co-op stays in sync.
- **Alternatives**: Two independent bools — previous, allowed both on; player-only export — misses hot-join P2 unless duplicated; runtime HUD toggle — deferred.
- **Status**: decided (in-codebase)

### Dash vault redirect (playtest)
- **Decision**: When `orb_redirect_mode == DASH_VAULT`, mid-combat steer is **dash into a flying orb**. While dashing, `prepare_vault_dash` tests the **remaining dash ray** against orbs within `vault_catch_radius` (**28 px**) and **commits** to the soonest orb, clamping remaining distance to the far-side landing (`orb + dash_dir * (orb_radius + body_radius + gap)`). The dash **keeps moving** (afterimages, i-frames) until the player reaches that landing; then `try_finish_vault_dash` calls `end_and_clear_cooldown()` and starts the aim window (light place + `CollisionSeparation` only to settle, not a mid-dash teleport). Orb enters vault hold while staying `FLYING`: `freeze`, zero velocity, trail off, instigator + **1 s** grace. While vaulting, **player movement and dash are locked**. Chevron follows aim for up to **`vault_hold_duration` (0.5 s)** in **360°** (`vault_aim_half_angle_degrees` default **180**). **Press dash again** during the window to fire early along aim and `force_state("dash")` into a real dash (starts the 1.5 s cooldown). Dash-away direction prefers **WASD / left stick** when held (faces that vector before the Dash state starts); with no move input, falls back to last facing (usually the inbound vault dash). That same orb is ignored for vault catch for **`vault_recatch_cooldown` (0.35 s)** so dash-away does not immediately re-grab it (other orbs still catchable). Timeout auto-fires without a follow-up dash and **refunds** dash cooldown. Fire calls `deflect()` (**+10%** boost). One vault at a time. Attack bat is off in this mode.
...
- **Tether feel**: playtest mid-combat steer is toggled by desert **`orb_redirect_mode`**: **Attack Bat** or **Dash Vault** (mutually exclusive). Orbit capture remains parked (`capture_enabled`); entity punch-through is the default (desert export can enable bounce). Vault early-fire is **dash-again** (spends dash); timeout refunds dash CD.
- **Attack cooldown / charges**: Attack bat mode **0.35 s** CD (no dash refund); dash-again early vault fire starts dash CD; timeout auto-fire refunds dash CD; tether post-release cooldown 0.25s; melee parked.
