...
- **Decision**: The former `tether` input action is renamed **`pickup`**. Dash is on **Space / R2**. **Attack** (LMB) is parked. Live ritual remaps pad **X → pickup**, adds **`upgrade`** (Y/Triangle / F) and **`ritual_cancel`** (B/Circle / C). Glyph throw stays on pickup; Attack remains parked.
- **Why**: Separates loot/circle/ritual verbs from combat Attack; pad X for pickup matches the live ritual socket verb.
- **Alternatives**: Keep throw on Attack — previous; keep Y as pickup — previous, remapped for live ritual.
- **Status**: decided (in-codebase)
