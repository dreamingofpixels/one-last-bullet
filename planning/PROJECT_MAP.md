...
- `project/areas/level/level.gd` — director: bakes navigation, starts `EnemySpawner`, awaits parallel `begin_level()`; applies `RunStartConfig` to the circle then launches configured orbs (default **Ghost + Contagion + Conduit**) from `%SummoningCircle` with a brief mutual collision exception (~0.2 s) so co-spawned orbs fan out; on kills rolls **glyph** spawn via orb `glyph_drop` or `glyph_drop_chance` export; weighted glyph rarity **70 / 25 / 5**; `SummoningCircle.ritual_started` → live circle UI (no pause); `new_blank_orb_requested` / `transform_requested` from circle; `pause` → `%RitualMenu.open_inspect`; co-op P2 hot-join; win/lose when all players dead; orb tether/time-slow hooks
...
- `project/objects/summoning_circle/summoning_circle.tscn` + `.gd` — floor circle with `%DepositArea` (radius 22), `%InfoProximityArea` (radius 64, player-only, OrbInfo visibility), `%ManaPoolLabel`, `%ArcaneParticles`, `%OrbInfo` (`%NameLabel` + `%EffectLabel` + stats), `%GlyphSlots` / `%GlyphSocket1-3` / `%GlyphIcon1-3` (`socket_interact_radius` 12 for Cross place/remove; GlyphSlots use `z_as_relative = false` like the floor sprite so they draw behind the player), `%BuyBlankOrb`; owns `mana_pool` (no glyph inventory); **`MAX_ORBS` 5**; live ritual: Square/`activate` arms capture, Triangle/`upgrade` buys or Transform/bake; signals `ritual_started` / `ritual_ended` / `new_blank_orb_requested` / `transform_requested`; group `summoning_circle`
