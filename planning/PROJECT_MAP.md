...
- `project/entities/orbs/blank/blank_orb.tscn` + `blank_orb.gd` (`class_name BlankOrb`) — flying rigid mask is **world|orb** (plus player|enemy when `bounce_off_entities`); scripted bounce; **`glyph_drop`** + **3 sparse glyph slots** (`apply_glyph` / `apply_glyph_at` / `has_glyph_at` / `socketed_count` / `remove_glyph`, `get_stat_snapshot`); **`begin_circle_capture`** / **`release_from_circle`** / **`assume_circle_capture`** for summoning ritual; weight knockback gated to `enemies` group; …
