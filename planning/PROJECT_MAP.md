...
│   ├── collision_separation.gd      Static shape-query helpers: is_clear / resolve_along / separate (dash unstick, player watchdog, enemy spawn depenetration)
    ...
    │   │   ├── controls.gd         Per-player input abstraction (move/aim/attack/pickup/dash)
    │   │   └── states/
    │   │       ├── idle.gd
    │   │       ├── walk.gd
    │   │       ├── attack.gd
...
- `project/entities/player/controls.gd` — `class_name Controls`; `apply_player_index(index)`; `uses_mouse()` (P1 only); `get_move_vector()`, `get_aim_vector(origin)`, `get_throw_aim_vector()` (right stick else move dir), `is_attack_just_pressed()`, `is_pickup_just_pressed()` / `is_pickup_pressed()` / `is_pickup_just_released()`, `is_dash_just_pressed()`
- `project/entities/player/states/idle.gd` — stops movement (yields while knockback active); plays `idle` when not mid-`attacking`; transitions to walk, dash; **Attack parked** (no press handling); movement locked while tethering / channeling / **vaulting**; dash blocked while vaulting (vault dash-again handled in `OrbTetherComponent._update_vault`)
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()` (yields while knockback active); plays `moving` and `face(dir)` when not mid-`attacking`; transitions to idle, dash; **Attack parked** (no press handling); forces idle while tethering / channeling / **vaulting**; dash blocked while vaulting
- `project/entities/player/states/attack.gd` — snapshots aim, `play("attacking")` + starts `AttackComponent`; returns to idle when swing ends (reachable only if `melee_enabled` and Attack is re-enabled)
...
- `project/objects/summoning_circle/summoning_circle.tscn` + `.gd` — floor circle with `%DepositArea` (radius 22, activate / deposit / capture), `%InfoProximityArea` (radius 64, player-only; OrbInfo, BuyBlankOrb, **Triangle buy/Transform/bake**, **Circle cancel**), `%ManaPoolLabel`, `%ArcaneParticles`, `%OrbInfo` (`%OrbNameLabel` + `%EffectLabel` + stats; `%FireHint` / `%WaterHint` / `%AirHint` / `%EarthHint` on each element row — shown only at exactly two socketed glyphs via `OrbRecipes.hints_for`; at three playable glyphs OrbInfo name/effect/stats preview the specialist), `%GlyphSlots` / `%GlyphSocket1-3` / `%GlyphIcon1-3` (`socket_interact_radius` 16 for L2 place/remove; sockets ~33 px from center, outside DepositArea; GlyphSlots use `z_as_relative = false` like the floor sprite so they draw behind the player), `%BuyBlankOrb` (idle / waiting, only while a player is in InfoProximityArea); owns `mana_pool` (no glyph inventory); **`MAX_ORBS` 5**; live ritual: Square/`activate` arms capture, Triangle/`upgrade` buys or Transform/bake then **launches** (Transform via `commit_transformed_orb`; bake then `release_orb`; capture grace on launch); signals `ritual_started` / `ritual_ended` / `new_blank_orb_requested` / `transform_requested`; group `summoning_circle`
...
| `attack` | left click — **parked** | — (empty; parked) |
| `pickup` | right click, gamepad **L2** (device 0, axis 4) | gamepad **L2** device 1 |
| `activate` | **E**, gamepad **Square** (device 0, button 2) — arm circle for capture | gamepad **Square** device 1 |
| `upgrade` | **F**, gamepad **Y / Triangle** (device 0) — buy blank / Transform/bake | gamepad **Y** device 1 |
| `ritual_cancel` | **C**, gamepad **B / Circle** (device 0) | gamepad **B** device 1 |
| `dash` | Space, gamepad **R2** (device 0, axis 5) | gamepad **R2** device 1 |
...
- Player input always goes through the `Controls` node's `PlayerAction` children; never hardcode action strings in state or component scripts. `apply_player_index(n)` appends `""` / `"_2"` / `"_3"` / `"_4"` suffix at `_ready` time. Actions: move / aim / attack / **pickup** / dash.
