...
- Mid-combat steer *(playtest)*: get within **48 px** of a flying orb — all in-range orbs show focus; the **closest** shows an aim arrow. Press **Attack** to redirect the closest in-range orb along aim (no melee swing, no +10% boost; resets dash cooldown; shares ~0.35 s attack cooldown). Orb **capture / orbit tether** is parked (`capture_enabled = false`); code kept. Flying orbs **punch through** player and enemy bodies (and each other) while still dealing damage. Post-redirect grace lasts **1 s** (ghostly orb tint + shrinking halo). Arc-contact deflect of the orb is parked (code kept). Out-of-range Attack still knocks enemies and has a short cooldown (~0.35s).
