...
- **Decision**: The former `tether` input action is renamed **`pickup`** (RMB / gamepad Y; P2 `pickup_2`). Dash is on **Space / R2** (R2 removed from Attack so the trigger is dash-only). **Attack** (LMB / gamepad X) is parked — Idle/Walk no longer handle Attack presses; melee and proximity redirect remain behind flags. **Pickup** grabs a glyph in focus range; a second press while carrying **throws** along aim (no Attack cooldown). Pickup still activates the summoning circle when standing in `DepositArea`. Ritual menu cancel uses `pickup`.
- **Why**: Separates loot/circle verbs from combat Attack; R2-as-dash matches the controller remap; Attack parked cleanly while vault early-fire moves to dash-again.
- **Alternatives**: Keep throw on Attack — previous; keep tether action name — mismatched with glyph-first use; share R2 on Attack and Dash — double-fires.
- **Status**: decided (in-codebase)
