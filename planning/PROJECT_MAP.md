...
- `project/entities/player/states/idle.gd` — stops movement (yields while knockback active); plays `idle` when not `is_attack_busy()`; transitions to walk, dash; movement locked while tethering / channeling / **vaulting** / **attack charging**; dash blocked while vaulting or attack charging (vault dash-again handled in `OrbTetherComponent._update_vault`); Attack bat polled in tether `_process`
- `project/entities/player/states/walk.gd` — moves from `controls.get_move_vector()` (yields while knockback active); plays `moving` and `face(dir)` when not `is_attack_busy()`; transitions to idle, dash; forces idle while tethering / channeling / **vaulting** / **attack charging**; dash blocked while vaulting or attack charging; Attack bat polled in tether `_process`
...
- Player facing: 8-way logical facing; visuals use authored `<action>_sw` / `<action>_nw` (`idle` / `moving` / `attacking` / `attack_around`) with `flip_h` for SE/NE; missing NW falls back to SW; `DirectionalSpriteComponent` owns octant snap, cardinal→diagonal mapping, east flip, and charge pause-hold; redirect/throw call `Player.play_attack_visual()` (dwarf spin uses `play_attack_around_visual()`).
