...
- **Alternatives**: Square activate / Triangle buy+transform — previous; Triangle toggle on/off with Square Transform — rejected, because the start button would dump the orb at the 3-glyph climax; Triangle activate+buy — previous, blocked buy while idle; refund mana on waiting-cancel — rejected for now (cancel is “wrong time,” not a free probe).
- **Status**: decided (in-codebase)
