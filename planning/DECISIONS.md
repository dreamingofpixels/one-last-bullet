...
- **Decision**: Destructible cliff segments are a dedicated `Cliff` breakable (`cliff.tscn` / `cliff.gd`). Authors pick `kind` (`corner_SW` / `corner_SE` / `wall_1` / `wall_2`) and may preview `frame_index` in the inspector; an `@tool` apply sets the vertical-strip `Sprite2D` (`hframes` 1 / `vframes` per sheet: 3 corner / 4 wall) plus body/hitbox rects. Does **not** call `LevelObject._ready()` / random `LevelObjectVariant`. Default HP **80**. Optional collision size/offset overrides. **At runtime**, `frame_index` is overwritten from 32 px neighbors (group `cliffs`; left / right / above, 4 px slop). `Cliff.refresh_all_frames` runs deferred on ready and when any cliff is destroyed (dying cliff leaves `cliffs` immediately and is skipped if queued for deletion so neighbors do not still see it). Rules: SW corner uses above+right (0 / 1 / else 2); SE uses above+left; walls use left+right (0–3).
- **Why**: Kind is placement authorship; frame must match living neighbors so destroying a segment reveals the correct cut faces without hand-wiring each instance. Editor frame remains for layout preview before play.
- **Alternatives**: Manual `frame_index` only — drifts as soon as anything breaks; TileMap terrain peering — cannot carry per-piece HP / destroy VFX; random `LevelObjectVariant` — wrong for authored cliff shapes.
- **Status**: decided (in-codebase)
