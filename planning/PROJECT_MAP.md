...
- `project/entities/orbs/vulcano/lava_globe.tscn` + `lava_globe.gd` (`class_name LavaGlobe`) — group `lava_puddles`; fly → splat → puddle → expire; bottom-origin + north sort bias for `%WorldYSort`; puddle Area2D applies 1 Burn/s to enemies then disables on expire; source = owning Vulcano
